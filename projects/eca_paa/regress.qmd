...
# util$zip_pack("tmp/eca_paa_data_v240315.zip", dp$opath)
# util$zip_unpack("tmp/eca_paa_data_v240315.zip", overwrite = TRUE)
...
tmp <- df[df$cbsa_rural_category == "rural" & df$eca_cluster_category != "Cluster Sink", ] 
tmp %>%   
  st_drop_geometry() %>% 
  # select(params$reg_factors) %>% 
      select(where(is.numeric)) %>% 
  summary() 

for(i in c(params$reg_factors)){
      dp$viz$density_dist_plot(tmp, i) %>% 
        print(.) %>% suppressWarnings()
}
 dp$viz$density_dist_plot(tmp, "max_trade_share") %>% suppressWarnings()
 dp$viz$density_dist_plot(tmp, "eca_center_distance") %>% suppressWarnings()
 
test <- dp$call_dist_mat(params$year) %>% 
  drop_units() %>% 
  as.data.frame.table() %>% 
  `colnames<-`(c("rows", "cols", "distance")) %>% 
  mutate(sample = c("population")) %>% 
  {.[.$rows %in% df[df$cbsa_rural_category == "rural" & df$eca_cluster_category != "Cluster Sink", ]$place, ]} %>% 
  {.[.$distance != 0, ]} %>% 
  mutate(distance = dp$meters2miles(distance))
test2 <- tmp %>% 
  st_drop_geometry() %>% 
...
dp$viz$density_dist_plot(tmp, "eca_center_distance") %>% suppressWarnings()
dp$viz$density_dist_plot(test2, "distance") 
dp$viz$density_dist_plot(test2, "distance", "sample")
}
