...
params:
  year: 2012
  year_range: !expr c(2012:2014) # or c(2012, 2015, 2017)
  ...
  show_corr: TRUE
  show_test_regression: TRUE
  show_lm_regression: TRUE
  show_lm_cluster_regression: TRUE
  reg_factors: !expr c("gross_output", "population", "employment", "jobs", "laborforce_part_rate", "unemp_rate", "payroll", "wage", "income_rate", "establishments", "entry_rate", "exit_rate", "poverty_rate", "ypll75", "highschool_attainment_rate")
  reg_factor_funs: !expr c("call_gross_output","call_population", "call_employment","call_unemp_rate", "call_laborforce_rate", "call_jobs", "call_establishments", "call_payroll", "call_wage", "call_exit_rate", "call_entry_rate", "call_income_rate", "call_poverty_rate", "call_ypll75", "call_highschool_attainment_rate")
...
# TODO: Diff in diff
# TODO: center to center distances and share weighted distance
```
...
# test <- dp$growth_rate(start_year = 2010, end_year = 2015, function_name = dp$call_county_population,  bus_data = "census")

x <- dp$temporal_permutations(params$year_range)
for (y in 1:nrow(x)){
  paste(x[y,1], x[y,2]) %>% print()
  for (i in params$reg_factors){
    dp$growth_rate(x[y,1], x[y,2], dp$call_employment) %>% head() %>% print()
  }
}
...
#       cat('\n')
#       cat('####', i, ' \n')
#       print(dp$viz$density_dist_plot(., i))
#       frame()
#       cat('\n')
#     }
#   }
