...
if (params$show_lm_cluster_regression) {
  ...
  data = df[df$cbsa_rural_category == "rural" & df$eca_cluster_category != "Cluster Sink", ]
  
  fm <- as.formula(paste(i, "~", x1, "*", x2))
     # 
     # specif[[i]] <- lm.cluster(fm,
     #          data = df[df$cbsa_rural_category == "rural" & df$eca_cluster_category != "Cluster Sink", ],
     #          cluster = "eca_membership")
     ...
     # specif[[i]] <- df[df$cbsa_rural_category == "rural" & df$eca_cluster_category != "Cluster Sink", ] %>% 
     #   {lm.cluster(.[[i]] ~  .[[paste0("sink_value_", i)]] + .[["eca_center_distance"]], cluster = .[["eca_membership"]])} %>% 
     #  dp$round_stats()
    # specif[[i]]$outcome <- paste0("source_", i)
    # specif[[i]]$term[2] <- paste0("sink_", i)
