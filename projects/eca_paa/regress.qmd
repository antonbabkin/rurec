...
dp$viz$hist_dist_plot(df[df$eca_cluster_category == "Destination", ], "cluster_members_count", adaptive_scaling = F) + labs(x = "Number of counties") %>% print()
...
dft[dft$distance > 0 & dft$sample == "ECA source-destination", ] %>% dp$viz$density_dist_plot(., "distance", adaptive_scaling = F) +scale_x_continuous(labels = label_comma(scale = 100))
