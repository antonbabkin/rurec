...
comp_map = data_map %>%
  mutate(comp = case_match(
    metro_micro_sink,
    "Sink and Metro " ~ "CBSA destination",
    "Sink and Micro" ~ "CBSA destination",
    "Metro only" ~ "CBSA source",
    "Micro only" ~ "CBSA source",
    "Sink only" ~ "Rural destination",
    NA ~ "Rural source"
  ))

tmp <- scale_fill_discrete
scale_fill_discrete <- function(...) scale_fill_manual(
  name = "ECA-CBSA Comparison", 
  values = c(
    "CBSA destination"="#1f78b4", 
    "CBSA source"="#a6cee3",
    "Rural destination"="#33a02c", 
    "Rural source"="#b2df8a"))
dp$viz$cat_choro_map(comp_map, "comp", interactive = F, legend = T)
scale_fill_discrete <- tmp
