...
```{r}
dataprep_misc$call_population() |> count(year)
```
...
```{r}
# uncomment as needed
# util$zip_pack("tmp/eca_paa_data_v240226.zip", dp$opath)
...
```{r}
dp$call_cbsa_conc(2013)

dp$call_cbsa_delin_df(2013)
...
#   dp$call_population(year, "tidy_acs")
# }
...
#   dp$call_employment(year, bus_data = "ers")
# }
...
```{r}
dp$call_employment(2002, bus_data = "cbp_raw") |> head()

# create full cache
for (year in 2002:2021) {
  dp$call_employment(year, bus_data = "cbp_raw")
}
...
```{r}
dp$call_establishments(2002, bus_data = "cbp_raw") |> head()

# create full cache
# for (year in 2002:2021) {
#   dp$call_establishments(year, bus_data = "cbp_raw")
# }
...
# create full cache
# for (year in 2002:2021) {
#   dp$call_payroll(year, bus_data = "cbp_raw")
# }
...
```{r}
dp$call_wage(2012) |> head()

# TODO: fix this
dp$growth_rate(2012, 2017, dp$call_wage) |> head()
```
...
```{r}
dp$call_unemp_rate() |> head() |> count(year)
df_unemp_rate |> head()
...
```{r}
dp$call_highschool_attainment_rate(2022) |> head()
...
```{r}
dp$call_poverty_rate(2012)
# TODO: review after Austin's commit
```
## RUCC
```{r}
df_RUCC <- dp$call_RUCC() %>%
  mutate(rural = ifelse(RUCC_2013 > 3, "non-metro", "metro")) %>%
  select("place"=FIPS, "RUCC"=RUCC_2013, rural)
df_RUCC |> head()
```
