...
df <- dp$call_proj_df(params$year) %>% 
  ...
  mutate(income_rate = income_rate) %>%
...
```{r}
# discrete histogram of ECA cluster counts 

dp$viz$hist_dist_plot(df[df$eca_cluster_category == "Destination", ], "cluster_members_count", adaptive_scaling = F) + labs(x = "Number of counties")

```

```{r}
# density distribution trade share maximium value

dp$viz$density_dist_plot(df, "max_trade_share") 

```

```{r}
# Distribution of distance (100's of miles, logarithmic scale) from ECA source counties to their respective destinations, compared with the distribution between each possible pair of counties
...
```{r}
# Distance in miles from ECA source counties to their respective destinations
  df %>% 
    mutate(eca_center_distance = replace(eca_center_distance, which(eca_center_distance == 0), NA)) %>% 
  dp$viz$nominal_choro_map(., "eca_center_distance", interactive = F)

```


```{r}
# Comparing ECA and CBSA county assignments
  ...
  p <- dp$viz$cat_choro_map(df, "metro_micro_destination", interactive = F, legend = T)
    print(p)
...
# PLay with ggplot ECA and CBSA county assignment comparrisons 
if (FALSE){
  tmp <- scale_fill_discrete
    scale_fill_discrete <- function(...) scale_fill_manual(values = c(
      "ECA dest, own CBSA center" = "#33a02c",
      "ECA source, non-CBSA" = "#1f78b4",
      "ECA source, same CBSA" = "#f5f2a3",
      "ECA source, other CBSA" = "#f5cba3" ,
      "other ECA, same CBSA" = "grey60",
      "outside" = "#f2f0ef"))
    

ECA_n <- "27053"
cbsa_n <- df[df$place == ECA_n,]$cbsa_of_eca

df[df$STATE %in% c("MN", "WI"),] %>%
  mutate(
    comp = ifelse(
      (place == ECA_n & CBSA_CODE == cbsa_n & eca_central_out_xtab == "Central Sink"), "ECA dest, own CBSA center",
      ifelse(
   ...
   dp$viz$cat_choro_map(., "comp", interactive = F, legend = F) + guides(fill = guide_legend(title = "Title"))
