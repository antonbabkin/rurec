...
params:
  year: 2012
  year_range: !expr c(2012, 2016) # or c(2012, 2015, 2017) or c(2010:2020)
  ...
  reg_factor_funs: !expr c("call_county_establishments", "call_county_wage", "call_county_jobs", "call_county_payroll", "call_county_employment", "call_county_poverty_rate", "call_county_income_rate", "call_county_population")
  reg_factor_bus_data: !expr c("cbp_imp", "cbp_imp", "cbp_imp", "cbp_imp", "ers", "saipe", "bea_profile", "tidy_acs")
...
```{r}
# Build collection of data input functions

call_prosperity_outcomes <- function(year){
  dl <- list(
    # Gross Domestic Product (GDP) in 1,000's of dollars total
    dataprep$call_county_gdp(year = year, bus_data = "bea_rea", price_level = switch(1, "nominal", "real")),
    # Population total
    switch(3, "census", "bea_profile", "tidy_acs") %>% 
      dataprep$call_county_population(year = year, bus_data = .),
    # Laborforce Participation rate
    dataprep$call_county_laborforce_rate(year = year, bus_data = "tidy_acs"),
    # Highschool Attainment rate
    dataprep$call_county_highschool_attainment_rate(year = year, bus_data = "tidy_acs"),
    # Years of Potential Life Lost per 100,000 
    dataprep$call_county_ypll75(year = year, bus_data = "chr") %>% 
      mutate(ypll75 = ypll75/100000),
    # Net Migration rate
    dataprep$call_net_migration(),
    # Poverty rate
    dataprep$call_county_poverty_rate(year = year, bus_data = "saipe"),
    # Employment total
    switch(2, "bea_profile", "ers") %>% 
      dataprep$call_county_employment(year = year, bus_data = . ),
    # Unemployment rate
    dataprep$call_county_unemployment_rate(year = year, bus_data = "ers"),
    # Income per capita in 1,000's of dollars total
    dataprep$call_county_income_rate(year = year, bus_data = "bea_profile") %>% 
      mutate(income_rate = income_rate/1000),
    # Establishment entry rate
    switch(1, "bds", "infogroup") %>% 
      dataprep$call_county_entry_rate(year = year, bus_data = .),
    # Establishment exit rate
    switch(1, "bds", "infogroup") %>% 
      dataprep$call_county_exit_rate(year = year, bus_data = .),
    # Number of Establishment total
    switch(1, "cbp_imp", "cbp_raw", "infogroup") %>% 
      dataprep$call_county_establishments(year = year, bus_data = .),
    # Number of Jobs total
    switch(1, "cbp_imp", "cbp_raw") %>% 
      dataprep$call_county_jobs(year = year, bus_data = .),
    # Annual Payroll in 1,000's of dollars total
    switch(1, "cbp_imp", "cbp_raw") %>% 
      dataprep$call_county_payroll(year = year, bus_data = .),
    # Annual Wages in 1,000's of dollars total
    bus_data = switch(1, "cbp_imp", "cbp_raw") %>% 
      dataprep$call_county_wage(year, bus_data = .),
    # Gross (Commodity) Output in 1,000's of dollars total
    # Intermediate (Domestic-Commodity) Supply in 1,000's of dollars total
    # Intermediate (Domestic-Commodity) Demand in 1,000's of dollars total
    # Excess (Detail level-Domestic-Commodity) Supply in 1,000's of dollars total
    # Excess (Detail level-Domestic-Commodity) Demand in 1,000's of dollars total
    bus_data = switch(1, "cbp_imp", "cbp_raw", "infogroup") %>% 
      place_output$call_extraction_table(year, ., ilevel = "det", class_system = "commodity", paradigm = "domestic", spatial = F) %>% 
      select(-extract)
  )
  df = dl[[1]]
...
call_space_df <- function(year){
  df <- util$year2cbsa(year) %>% 
    geography$pubdata$get_cbsa_delin_df(.) %>% 
    ...
    left_join(geography$call_geog(year), ., by = "place") %>% 
    ...
    group_by(CBSA_CODE) %>% 
    mutate(cbsa_members_count = n(), .after = CBSA_CODE) %>% 
    ungroup() %>% 
    {left_join(., dataprep$call_ruc(year), by = "place")}
  return(df)
}

call_eca_space_df <- function(year){
  df_eca <- trade_flows$call_trade_flows("all_industries") %>%
    {sweep(., 1, rowSums(.), "/")} %>%
    connectedness$apply_absorption_metrics() %>%
    connectedness$apply_absorption_algorithm(., threshold = 0) %>%
    select(place, cluster_category, eca_membership, max_alpha, match) %>%
    rename(eca_cluster_category = cluster_category, max_trade_share = max_alpha, max_trade_place = match ) %>%
    mutate(eca_cluster_category = ifelse(eca_cluster_category == "Cluster Source", "Source", "Destination") )
  df <- left_join(call_space_df(year), df_eca,  by = "place")  %>%
    na.omit() %>% 
  ...
  dist_mat <- geography$call_dist_mat(year)
  df <- df %>% 
    # Distance to ECA destination in 100's of miles
    mutate(eca_center_distance = apply(df, 1, function(x){drop_units(dist_mat[x$place, x$eca_membership]) %>% {util$meters2miles(.)/100}}) ) %>% 
    ...
    mutate(cbsa_by_eca = ifelse(eca_central_out_xtab == "Central Source", "only CBSA", 
                                ifelse(eca_central_out_xtab == "Central Destination", "ECA and CBSA", 
                                       ifelse(eca_central_out_xtab %in% c("Rural Destination", "Outlying Destination"), "only ECA", 
                                              NA))), .before = eca_membership) %>% 
    mutate(metro_micro_destination = ifelse(METRO_MICRO == "metro" & eca_cluster_category != "Cluster Destination", "Metro only",
                                         ifelse(METRO_MICRO == "micro" & eca_cluster_category != "Cluster Destination", "Micro only",
                                                ifelse(METRO_MICRO == "metro" & eca_cluster_category == "Cluster Destination", "Destination and Metro ",
                                                       ifelse(METRO_MICRO == "micro" & eca_cluster_category == "Cluster Destination", "Destination and Micro",
                                                              ifelse(METRO_MICRO == "rural" & eca_cluster_category == "Cluster Destination", "Destination only",
                                                              NA))))), .before = eca_membership)
...
call_proj_df <- function(year){
  ...
  x <- util$temporal_permutations(params$year_range)
  tmp <- df$place %>% 
    as.data.frame() %>% 
    `colnames<-`(c("place"))
  for (y in 1:nrow(x)){
    ...
    for (i in params$reg_factor_funs){
       tmp <- dataprep$growth_rate(x[y,1], x[y,2], dataprep[[i]], bus_data = params$reg_factor_bus_data[[match(i, params$reg_factor_funs)]]) %>%
         mutate(grow_rate = replace(grow_rate, which(abs(grow_rate) == 200), NA)) %>%
         rename_at('grow_rate', ~paste0(sub("^call_county_", "", i), "_gr")) %>%
  ...
  df <- circularity$call_circularity_metrics(
    year = params$year,
    ilevel = switch(1, "det", "sum", "sec"),
    class_system = switch(2, "industry", "commodity"),
    paradigm = switch(2, "factor", "domestic", "capital"),
    bus_data = switch(1, "cbp_imp", "cbp_raw", "infogroup"),
    spatial = FALSE) %>% 
...
# build main project dataframe
df <- call_proj_df(params$year) 

# TODO: do we need to worry about "02158" "15901" "46102" "51901" "51903" "51907" "51911" "51913" "51918" "51919" "51921" "51923" "51929" "51931" "51933" "51939" "51941" "51942" "51944" "51945" "51947" "51949" "51951" "51953" "51955" "51958" "02201" "02232" "02994" "02995" "02996" "01998" "01999" "02010" "02140" "02231" "02280" 

# 2012 %>% {setdiff(call_prosperity_outcomes(.)$place, call_space_df(.)$place)}
...
# dummy check for user variable choice against dataset availability
stopifnot("variable selection and names do not match" = all(rf %in% names(df)))


# row normalized trade value matrix of potential excess trade flows between all places
tm <- trade_flows$call_trade_flows("all_industries") %>% 
  util$row_normalize()
...
pn <- pn %>% 
  {geography$call_bprox_mat(year = params$year, queen = TRUE)[., .]} %>% 
  {rownames(.)[rowSums(.) > 1]}
...
if (params$build_w) {
  W_list <- geography$imped_list(
    pn_dim = pn, 
    dmat = geography$call_dist_mat(year = params$year) )
}  
...
if (params$build_w) {
  ...
  if (file.exists(cache_path)) {
    ...
    saveRDS(W_list_alt, util$mkdir(cache_path))
  }
}
...
if (params$build_w) {
  ...
  if (file.exists(cache_path)) {
    ...
    W_list <- geography$imped_list(pn_dim = rural_places, dmat = geography$call_dist_mat(year = params$year))
    for (j in 1:length(W_list)) {
      W_list_alt_rural[[j]] <- W_list[[j]] %>%
        mat2listw(style = "M", zero.policy = TRUE)
    }
    saveRDS(W_list_alt_rural, util$mkdir(cache_path))
  }
}
...
if (params$build_w) {
  ...
  if (file.exists(cache_path)) {
    ...
    W_list <- geography$imped_list(pn_dim = nonrural_places, dmat = geography$call_dist_mat(year = params$year))
    for (j in 1:length(W_list)) {
      W_list_alt_nonrural[[j]] <- W_list[[j]] %>%
        mat2listw(style = "M", zero.policy = TRUE)
    }
    saveRDS(W_list_alt_nonrural, util$mkdir(cache_path))
  }
}
...
df_long <- util$temp_fun_recur_list(params$year_range, call_proj_df) %>% 
  bind_rows(., .id = "id_year") %>% 
  filter(place %in% rdf$place)

prf <- str_remove_all(grep("_gr*", params$reg_factors, value = T), '(_gr)$')

tmp <- df_long[c("id_year", "place", prf)] %>% 
  st_drop_geometry() %>% 
...
for (i in prf){
  visualization$density_dist_plot(df_long, i, "id_year", adaptive_scaling = T) %>% 
    print()
}
...
rdf %>%
    {for(i in rf){
      (visualization$density_dist_plot(., i, adaptive_scaling = F) + 
...
rdf %>%
    {for(i in rf){
      (visualization$density_dist_plot(., i, "cbsa_rural_category", adaptive_scaling = F) + 
        labs(x = coef_names[match(i, rf)])) %>%
        print(.)
      }
    }
...
rdf %>%
    {for(i in rf){
      (visualization$density_dist_plot(., paste0(i, "_", twg), adaptive_scaling = F) + 
...
rdf %>%
    {for(i in rf){
      (visualization$density_dist_plot(., paste0(i, "_", twg), "cbsa_rural_category", adaptive_scaling = F) + 
        labs(x = paste("Trade Weighted", coef_names[match(i, rf)]))) %>%
        print(.)
    }
  }
...
ihs_trans <- function(){trans_new("ihs", function(x){asinh(x)}, function(x){sinh(x)} )}
  ...
  for (i in params$reg_factors){
    visualization$diverge_choro_map(df, i, interactive = F) %>% 
      print()
  }
...
ihs_trans <- function(){trans_new("ihs", function(x){asinh(x)}, function(x){sinh(x)} )}
  ...
  for (i in params$reg_factors){
    visualization$diverge_choro_map(tmp, i, interactive = F) %>% 
      print()
  }
...
# map of data coverage
  ...
  scale_fill_discrete <- function(...) scale_fill_manual(values = c("Missing" = "black", 
                                                                    "Included" = "white"))
  visualization$cat_choro_map(tmp, "coverage", interactive = F, legend = T)
