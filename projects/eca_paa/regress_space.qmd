...
spec_df_regime <- function(spec_list, mn, cn){
  df <- data.frame()
  for (j in params$w_names){
    for (i in rf){
      estimate <-  summary(spec_list[[match(j, params$w_names)]][[i]])[["CoefTable"]][cn, 1]
      lower <- summary(specif[[match(j, params$w_names)]][[i]])[["CoefTable"]][cn, 1] - summary(specif[[match(j, params$w_names)]][[i]])[["CoefTable"]][cn, 2] * 1.959964
      upper <- summary(specif[[match(j, params$w_names)]][[i]])[["CoefTable"]][cn, 1] + summary(specif[[match(j, params$w_names)]][[i]])[["CoefTable"]][cn, 2] * 1.959964
      wmatrix <- j
      model <- mn
      indicator <- coef_names[match(i, rf)]
      df = rbind(df, data.frame(model, wmatrix, indicator, estimate, lower, upper))
...
coef_point_plot <- function(df, error = TRUE, id_variable = "wmatrix"){
  g <- df %>%
    (function(x) {
      x$model <- factor(x$model)
      x$wmatrix <- factor(x$wmatrix, levels = c("none", params$w_names))
...
# dataframe containing only the pertinent variables and values used in the subsiquent analysis and regressions 
...
# In contrast to a Pearson correlation coefficient, the value of Moran’s I is not meaningful in and of itself. Its interpretation and the comparison of results among variables depend critically on the spatial weights used. Only when the same weights are used are results among variables directly comparable. To avoid this problem, a reliable comparison should be based on the z-values.
  ...
  cache_path = glue(proj_path, "moran_base.rds")
  if (file.exists(cache_path)) {
