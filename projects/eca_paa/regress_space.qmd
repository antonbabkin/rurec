...
point_plot <- function(df, error = TRUE){
  ...
  ggplot(aes(x=indicator, y=estimate, colour=wmatrix, shape=model)) +
    ...
    scale_color_manual(values=c("black", "#a6cee3", "#1f78b4", "#b2df8a", "#33a02c", "#fb9a99", "#e31a1c", "#fdbf6f", "#ff7f00")) +
  ...
  if (error){g <- g + geom_errorbar(aes(x=indicator, y=estimate, ymin=lower, ymax=upper), width=.2, position = position_dodge(width=.3), data = df)}
  return(g)
}
  ...
  specif[[j]] %>% 
    ...
    {msummary(., 
              ...
              coef_map = c("(Intercept)_1" = "(Intercept): non-rural",
                           ...
                           "trade_weighted_growth_2" = "Trade Weighted Growth: rural") )} %>% print()
}
...
g <- tmp %>% filter(model != "OLS_alt") %>% filter(model != "OLS_w") %>% 
ggplot(aes(x=indicator, y=estimate, colour=wmatrix, shape=model)) +
  geom_point_interactive(position = position_dodge(width=.3)) +
  scale_color_manual(values=c("black", "#a6cee3", "#1f78b4", "#b2df8a", "#33a02c", "#fb9a99", "#e31a1c", "#fdbf6f", "#ff7f00")) +
  labs(x = "Growth Indicator", y = "Coeffecient Estimate", color = "W Matrix", shape = "Model") + 
...
if(FALSE){
g1 <- ggplot() +
  geom_point_interactive(aes(x=indicator, y=estimate, colour=wmatrix, shape=model), data = tmp[tmp$model == "OLS", ]) +
  geom_errorbar(aes(x=indicator, y=estimate, ymin=lower, ymax=upper), width=.2, data = tmp[tmp$model == "OLS", ]) +
  scale_color_manual(values=c("black", "#a6cee3", "#1f78b4", "#b2df8a", "#33a02c", "#fb9a99", "#e31a1c", "#fdbf6f", "#ff7f00")) +
  labs(x="Growth Indicator", y="Coeffecient Estimate", color="W Matrix", shape="Model") + 
  geom_hline(yintercept = 0) +
  theme_minimal() +
...
dp$girafe_plot(ggobj = g1)


g <- g1 + geom_point_interactive(aes(x=indicator, y=estimate, colour=wmatrix, shape=model), data = tmp[tmp$model %in% c("slm"), ])
dp$girafe_plot(ggobj = g)

g <- g1 + geom_point_interactive(aes(x=indicator, y=estimate, colour=wmatrix, shape=model), data = tmp[tmp$model %in% c("sem"), ])
dp$girafe_plot(ggobj = g)

g <- g1 + geom_point_interactive(aes(x=indicator, y=estimate, colour=wmatrix, shape=model), data = tmp[tmp$model %in% c("slm", "sem"), ])
dp$girafe_plot(ggobj = g)

g <- g1 + geom_point_interactive(aes(x=indicator, y=estimate, colour=wmatrix, shape=model), data = tmp[tmp$model %in% c("slm", "sem", "sem_w"), ])
dp$girafe_plot(ggobj = g)
