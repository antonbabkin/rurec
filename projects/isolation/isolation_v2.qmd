...
# 0. Load your dataset
# ---------------------------
# df_tf should have columns: from, to, flow
# df_tf <- read.csv("your_data.csv", stringsAsFactors = FALSE)

# Ensure character columns
df_tf$from <- as.character(df_tf$from)
df_tf$to   <- as.character(df_tf$to)

# ---------------------------
# 1. Optional: drop tiny flows
# ---------------------------
# min_flow <- 1000  # adjust threshold
# df_tf <- df_tf[df_tf$flow >= min_flow, ]

# ---------------------------
# 2. Prepare nodes
# ---------------------------
all_nodes <- unique(c(df_tf$from, df_tf$to))
n <- length(all_nodes)
node_idx <- setNames(seq_len(n), all_nodes)
...
for(i in seq_len(nrow(df_tf))){
  W[node_idx[[df_tf$from[i]]], node_idx[[df_tf$to[i]]]] <- 
    W[node_idx[[df_tf$from[i]]], node_idx[[df_tf$to[i]]]] + df_tf$flow[i]
}

# ---------------------------
# 4. Transition matrix with damping
# ---------------------------
P <- W
for(i in 1:n){
  row_sum <- sum(P[i,])
  if(row_sum == 0) P[i,i] <- 1 else P[i,] <- P[i,]/row_sum
}
...
# ---------------------------
# 5. Stationary distribution (power iteration)
# ---------------------------
pi <- rep(1/n, n)
for(iter in 1:5000){
  pi_new <- pi %*% P
  if(max(abs(pi_new - pi)) < 1e-12) break
  pi <- pi_new
}
pi <- pi / sum(pi)
...
# ---------------------------
persist <- function(S){
  num <- sum(pi[S] * rowSums(P[S,S,drop=FALSE]))
  den <- sum(pi[S])
  if(den == 0) return(0)
  return(num/den)
}
...
# 7. Singleton alpha
# ---------------------------
alpha_single <- sapply(seq_len(n), persist)
names(alpha_single) <- all_nodes

# Total flow (in + out) for tie-breaking
total_flow <- rowSums(W) + colSums(W)

# ---------------------------
# 8. Rank nodes by alpha_single with tie-breaking
# ---------------------------
# Ensure total_flow has names
names(total_flow) <- all_nodes

# Initial descending sort by alpha_single
ord <- names(sort(alpha_single, decreasing = TRUE))
profile_values <- alpha_single[ord]

# Find duplicated alpha values
dup_values <- unique(profile_values[duplicated(profile_values)])

# Break ties by total flow safely
for(val in dup_values){
  tied_nodes <- ord[profile_values == val]           # node names with same alpha
  tie_order <- tied_nodes[order(total_flow[tied_nodes], decreasing = TRUE)]  # order by total flow
  ord[profile_values == val] <- tie_order            # replace in ord
}

# ---------------------------
# 9. Build cumulative coreness profile
# ---------------------------
profile <- sapply(seq_along(ord), function(k) persist(node_idx[ord[1:k]]))
...
# 11. Compute CPI
# ---------------------------
baseline <- seq(1/n, 1, by=1/n)
CPI <- (mean(profile) - mean(baseline)) / (1 - mean(baseline))

# ---------------------------
# 12. Output
# ---------------------------
print(head(profile_df, 20))
cat("Core-Periphery Index (CPI):", CPI, "\n")

profile <- sapply(seq_along(ord), function(k) persist(node_idx[ord[1:k]]))
profile_df <- data.frame(FIPS = ord, coreness = profile, rank = seq_along(ord), stringsAsFactors = FALSE)
profile_df$rank <- rank(-profile_df$coreness, ties.method = "first")

profile_df = profile_df %>% 
  rename("place"="FIPS",
         "coreness_rank"="rank") %>% 
  mutate(FIPS = substr(place, 5, 9))

hist(profile_df$coreness)
...
# 12. Output
# ---------------------------
print(head(profile_df2, 20))
cat("Core-Periphery Index (CPI):", CPI, "\n")

profile <- sapply(seq_along(ord), function(k) persist(node_idx[ord[1:k]]))
profile_df2 <- data.frame(FIPS = ord, coreness = profile, rank = seq_along(ord), stringsAsFactors = FALSE)
profile_df2$rank <- rank(-profile_df2$coreness, ties.method = "first")

profile_df2 = profile_df2 %>% 
  rename("cz"="FIPS",
         "coreness_rank"="rank") %>% 
  left_join(y=xwalk, by="cz") %>% 
  dplyr::select(place, "alt_cpi"=coreness)
...
g <- igraph::graph_from_data_frame(d = df_tf2, directed = FALSE)

# Calculate s-coreness using the brainGraph::s_core() function
weighted_coreness_scores <- brainGraph::s_core(g)

# Display the results
print(weighted_coreness_scores)

core_brain = as.data.frame(weighted_coreness_scores) %>%
  rownames_to_column(var = "place") %>% 
  mutate(core = ifelse(weighted_coreness_scores >= 208, 1,
                       ifelse(weighted_coreness_scores < 208, 0, NA))) %>% 
  dplyr::select(place, core)


df_tf3 = df_tf2 %>% 
  filter(flow > 500000) %>% 
  mutate(flow = log1p(flow))
...
core_brain2 = as.data.frame(weighted_coreness_scores) %>%
  rownames_to_column(var = "place") %>% 
  mutate(core2 = ifelse(weighted_coreness_scores >= 208, 1,
                       ifelse(weighted_coreness_scores < 208, 0, NA))) %>% 
  dplyr::select(place, core2)


core_brain = core_brain %>% 
  left_join(y=core_brain2, by="place")
...
core_brain3 = as.data.frame(weighted_coreness_scores) %>%
  rownames_to_column(var = "cz") %>% 
  left_join(y=xwalk, by="cz") %>% 
  dplyr::select(place, "alt_core"=weighted_coreness_scores)
...
core_brain4 = as.data.frame(weighted_coreness_scores) %>%
  rownames_to_column(var = "cz") %>% 
  left_join(y=xwalk, by="cz") %>% 
  dplyr::select(place, "alt2_core"=weighted_coreness_scores)
...
isolation = census_map  %>% 
  left_join(y=profile_df, by="place") %>% 
  left_join(y=profile_df2, by="place") %>% 
  left_join(y=core_brain, by="place") %>% 
...
```{r}
write.csv(iso2, "temp_data.csv")
# isolation = read.csv("temp_data.csv") %>% dplyr::select(-X)
```
...
temp = iso2 %>% 
  ...
  mutate(alt_core = ifelse(is.na(alt_core), 0, alt_core),
         econiso = ifelse(alt_core >= 10, 0,
                       ifelse(alt_core < 10, 1, NA)))
...
isolation %>%
  ...
  tm_layout(title = "Coreness", title.size = 2) +
  tm_shape(states) + 
  tm_borders(lwd=0.6)
...
ggplot(data = temp_nonmetro, aes(x = povch, y = quad_nonmetro, fill = quad_nonmetro)) +
  stat_density_ridges(
  ...
  labs(
    title = "Distributions of tract changes in poverty",
    subtitle = "2012-2023",
    x = "Change in poverty rate, 2012-2023",
    y = "Tract typology"
  ) +
  theme_ridges() +
  theme(
...
ggplot(data = temp_nonmetro, aes(x = emprtch, y = quad_nonmetro, fill = quad_nonmetro)) +
  stat_density_ridges(
    geom = "density_ridges_gradient",
    alpha = 0.8,
    quantile_lines = TRUE,
  ...
  labs(
    title = "Distributions of tract changes in employmnet rate",
    subtitle = "2012-2023",
    x = "Change in employment rate, 2012-2023",
    y = "Tract typology"
  ) +
  theme_ridges() +
  theme(
...
ggplot(data = temp_nonmetro, aes(x = pov_2023, y = quad_nonmetro, fill = quad_nonmetro)) +
  stat_density_ridges(
    geom = "density_ridges_gradient",
    alpha = 0.8,
    quantile_lines = TRUE,
...
ggplot(data = temp_nonmetro, aes(x = emprt_2023, y = quad_nonmetro, fill = quad_nonmetro)) +
  stat_density_ridges(
    geom = "density_ridges_gradient",
    alpha = 0.8,
    quantile_lines = TRUE,
...
ps_stat <- function(data, indices, g1, g2) {
  d <- data[indices, ]
  x <- d$pov_2012[d$quad_nonmetro == g1]
  y <- d$pov_2012[d$quad_nonmetro == g2]
...
ps_stat <- function(data, indices, g1, g2) {
  d <- data[indices, ]
  x <- d$emprt_2012[d$quad_nonmetro == g1]
  y <- d$emprt_2012[d$quad_nonmetro == g2]
...
ps_stat <- function(data, indices, g1, g2) {
  d <- data[indices, ]
  x <- d$povch[d$quad_nonmetro == g1]
  y <- d$povch[d$quad_nonmetro == g2]
...
ps_stat <- function(data, indices, g1, g2) {
  d <- data[indices, ]
  x <- d$emprtch[d$quad_nonmetro == g1]
  y <- d$emprtch[d$quad_nonmetro == g2]
...
merge messed up (cuz cz is trying to merge with fips)
...
ggplot(isolation, aes(x = coreness)) +
  geom_density(fill = "#6FA8DC", color = "black", alpha = 1.3) +
  theme_minimal() +
  labs(
    title = "Density Plot of Coreness",
    x = "coreness",
    y = "Density"
  )
...
# mx = median(isolation$coreness, na.rm=TRUE)
isolation %>%
  tm_shape() + 
  tm_polygons(col = "coreness", midpoint = 0.5, border.alpha = 0.0, style = "cont", 
...
isolation %>%
  tm_shape() + 
  tm_polygons(col = "alt_cpi", midpoint = 0.5, border.alpha = 0.0, style = "cont", 
              palette = c("#6FA8DC","white","#20124d")) +
  tm_layout(title = "Coreness", title.size = 2) +
  tm_shape(states) + 
  tm_borders(lwd=0.6)
...
isolation$core <- as.factor(isolation$core)
isolation %>%
  tm_shape() + 
  tm_polygons(
    col = "core",
    palette = c("0" = "#6FA8DC", "1" = "#20124d"), # Use named colors for clarity
    border.alpha = 0.0,
    title = "Core vs. periphery"
  ) +
  tm_layout(title = "Core vs. periphery states", title.size = 2) +
  tm_shape(states) + 
  tm_borders(lwd = 0.6)

mx = median(isolation$alt_core, na.rm=TRUE)
isolation %>%
  tm_shape() + 
  tm_polygons(col = "alt_core", midpoint = mx, border.alpha = 0.0, style = "cont", 
              palette = c("#6FA8DC","white","#20124d")) +
  tm_layout(title = "alt coreness", title.size = 2) +
  tm_shape(states) + 
  tm_borders(lwd=0.6)
...
Below, you are able to input variables in the economic isolation and
geographic isolation categories. Listing: **economic** --\> market share
(mksh) --\> trade capacity (trade_capacity) **geographic** --\>
trade-weighted distance (twd, logtwd) --\> share of exports within
(lsh50, lsh100, lsh200, lsh400)
