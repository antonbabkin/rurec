...
# TODO: use this variable to branch coreness calculation
coreness_type = "s" # "s" for weighted s-core, "k" for unweighted k-core
input_ptile = 0.50
# bootstrap size
# TODO: use these numbers in the test sections
ps_boot_n = 100
perm_boot_n = 100
...
ipath <- list(
  ...
  inflation = "projects/isolation/data/raw/inflation.csv",
  cola = "projects/isolation/data/raw/cola2.csv",
  xwalk_county_puma_2012 = "projects/isolation/data/raw/county_puma12.csv",
  xwalk_county_puma_2022 = "projects/isolation/data/raw/county_puma22.csv",
  ipums_ddi = "projects/isolation/data/raw/usa_00048.xml", 
  ipums_raw = "projects/isolation/data/raw/usa_00048.dat.gz",
  xwalk_puma_2012_2022 = "projects/isolation/data/raw/puma2010-to-puma2020.csv",
  xwalk_tract_puma = "projects/isolation/data/raw/2010_Census_Tract_to_2010_PUMA.txt",
  nhgis_emp_2000 = "projects/isolation/data/raw/nhgis0031_ts_nominal_tract.csv",
  nhgis_emp_2012 = "projects/isolation/data/raw/nhgis0032_ds192_20125_puma.csv",
  nhgis_emp_2022 = "projects/isolation/data/raw/nhgis0032_ds268_20235_puma.csv",
  xwalk_tract_2012_2022 = "projects/isolation/data/raw/tract1222.csv",
  xwalk_tract_2012_2022_alt = "projects/isolation/data/raw/xwalk_tract.csv",
  xwalk_county_cz = "projects/isolation/data/raw/xwalk_cz.csv",
  ruc_2013 = "projects/isolation/data/raw/ruralurbancodes2013.csv",
  ruca_2012 = "projects/isolation/data/raw/usda_ruca.csv",
  far_2010 = "projects/isolation/data/raw/usda_far.csv",
  xwalk_zip_tract = "projects/isolation/data/raw/zip_tract_092016.csv"
)

# output data files
dir.create("projects/isolation/data/proc/")
opath <- list(
  ccapov = "projects/isolation/data/proc/ccapov.rds",
  data = "projects/isolation/data/proc/data.rds"
)
...
```{r}
if (file.exists(opath$ccapov)) {
  # read cached data file
  ccapov = readRDS(opath$ccapov)
  cat(paste("read from cache:", opath$ccapov, "\n"))
} else {
  # create the data file

  inf_clean = read.csv(ipath$inflation)
  ...
  cola_clean = read.csv(ipath$cola) %>% 
    dplyr::select(-X) %>% 
    mutate(FIPS = sprintf("%05d", FIPS))
  
  xwalk12 = read.csv(ipath$xwalk_county_puma_2012) %>% 
  ...
  xwalk22 = read.csv(ipath$xwalk_county_puma_2022) %>% 
    filter(state != "State code") %>% 
  ...
  ddi <- read_ipums_ddi(ipath$ipums_ddi)
  pums_clean <- read_ipums_micro(ddi, data_file = ipath$ipums_raw)
  
  
  puma1222 = read.csv(ipath$xwalk_puma_2012_2022) %>% 
    filter(state != "State code") %>% 
  ...
  # save data file for later reads
  saveRDS(ccapov, opath$ccapov)
  cat(paste("saved to cache:", opath$ccapov, "\n"))
}
...
```{r}
xwalk_tpuma = read.csv(ipath$xwalk_tract_puma) %>% 
...
```{r}
nhgis_clean = read.csv(ipath$nhgis_emp_2000)
nhgis2_clean = read.csv(ipath$nhgis_emp_2012)
nhgis3_clean = read.csv(ipath$nhgis_emp_2022)


```

```{r}
xwalk_tract = read.csv(ipath$xwalk_tract_2012_2022) %>% 
  mutate(tract12 = str_pad(GEOID_TRACT_10, width = 11, pad = "0", side = "left"),
         tract22 = str_pad(GEOID_TRACT_20, width = 11, pad = "0", side = "left"))
```

```{r}
altwalk_tract = read.csv(ipath$xwalk_tract_2012_2022_alt) %>% 
...
```{r}
xwalk = read.csv(ipath$xwalk_county_cz) %>% 
...
if (file.exists(opath$data)) {
  data = readRDS(opath$data)
  cat(paste("read from cache", opath$data, "\n"))
} else {
  
  ruca_clean = read.csv(ipath$ruca_2012)
  far_clean = read.csv(ipath$far_2010)
  xwalk_trzip = read.csv(ipath$xwalk_zip_tract) %>% 
  ...
  data = geo %>% 
    ...
    mutate(econiso = ifelse(alt_core > ptile, 0, 1))
  ...
  saveRDS(data, opath$data)
  cat(paste("save to cache:", opath$data, "\n"))
...
# TODO: what is used for scatterplots below?
t2 = data
```
...
data %>%
  st_drop_geometry() %>%
  filter(ruca > 1) %>%
  mutate(outcome = pov_2023, far = as.logical(far)) %>%
  tidyr::drop_na(far, alt_core, outcome) %>%
  filter(outcome < 0.75) %>%
  # slice_sample(n = 5000) %>%
  # ggplot(aes(x = alt_core, y = outcome)) +
  ggplot(aes(x = alt_core, y = outcome, color = far)) +
  geom_point(alpha = 0.2, size = 0.5) +
  geom_smooth(method = "loess", formula = y ~ x) +
  theme_minimal() +
  labs(title = "Poverty rate") + xlab("s-coreness") + ylab(NULL)
```
...
data %>%
  st_drop_geometry() %>%
  filter(ruca > 1) %>%
  mutate(outcome = emprt_2023, far = as.logical(far)) %>%
  tidyr::drop_na(far, alt_core, outcome) %>%
  filter(outcome < 1, outcome > 0.25) %>%
  # slice_sample(n = 5000) %>%
  # ggplot(aes(x = alt_core, y = outcome)) +
  ggplot(aes(x = alt_core, y = outcome, color = far)) +
  geom_point(alpha = 0.2, size = 0.5) +
  geom_smooth(method = "loess", formula = y ~ x) +
  theme_minimal() +
  labs(title = "Employment rate") + xlab("s-coreness") + ylab(NULL)
```
...
```{r}
isolation %>%
# TODO: different modification for k-core and s-core
...
tmap_mode(mode = "plot")
  ...
  # Map
  temp_nonmetro %>%
    rename(Isolation = quad_nonmetro) %>%
  tm_shape() + 
    tm_polygons(
      fill = "Isolation",
      border.alpha = 0.0,
...
#     }
# 
#     # adaptive R: smaller groups → fewer replicates to save time
#     R_adj <- R
#     small_thresh <- 20
...
```{r}
# TODO: use actual numbers from the test above

t1 <- tribble(
  ~gr1, ~gr2, ~est, ~lo, ~hi,
  "econ",	"geo",	0.5578786,	0.5391862,	0.5768437,
  "econ",	"both",	0.5289097,	0.5118391,	0.5449887,
  "geo",	"both",	0.4693049,	0.4519635,	0.4870694,
  "econ",	"neither",	0.6132981,	0.600587,	0.6256634,
  "geo",	"neither",	0.564663,	0.551988,	0.5774617,
  "both",	"neither",	0.5912301,	0.5799038,	0.6022359
) %>%
  mutate(cmp = paste(gr1, ">", gr2))

t1

t1 %>%
  ggplot() +
  geom_point(aes(cmp, est)) +
  geom_errorbar(aes(cmp, ymin = lo, ymax = hi), width = 0.2) +
  scale_x_discrete(name = NULL) +
  scale_y_continuous(name = NULL, limits = c(0.25, 0.75), labels = scales::label_percent(scale_cut = scales::cut_short_scale())) +
...
t1 <- tribble(
  ~gr1, ~gr2, ~est, ~lo, ~hi,
  "econ",	"geo",	0.4444924,	0.4248617,	0.4630193,
  "econ",	"both",	0.4611243,	0.4450318,	0.4774179,
  "geo",	"both",	0.5145441,	0.4965124,	0.5333515,
  "econ",	"neither",	0.4147171,	0.4012746,	0.4279123,
  "geo",	"neither",	0.4731854,	0.4587063,	0.487573,
  "both",	"neither",	0.4584097,	0.4457513,	0.4708252
) %>%
  mutate(cmp = paste(gr1, ">", gr2))
