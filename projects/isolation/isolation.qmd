...
NMS1a = df_outsupdem %>% 
  dplyr::select(place, com_code, "NMS1a"=exsup)

NMS1b = df_outsupdem %>% 
  dplyr::select(place, com_code, "NMS1b"=exdem)

NMS1c = df_outsupdem %>% 
  group_by(com_code) %>% 
  dplyr::summarize(exsup = sum(exsup, na.rm=TRUE),
                   exdem = sum(exdem, na.rm=TRUE)
                   ) %>% 
  dplyr::select(com_code, "NMS1c"=exsup)
...
NMS2a = df_outsupdem %>% 
  dplyr::select(place, com_code, exsup) %>% 
  left_join(y=NMS1c, by="com_code") %>% 
  mutate(NMS2a = exsup/NMS1c) %>% 
  dplyr::select(place, com_code, NMS2a)

NMS2b = df_outsupdem %>% 
  dplyr::select(place, com_code, exdem) %>% 
  left_join(y=NMS1c, by="com_code") %>% 
  mutate(NMS2b = exdem/NMS1c) %>% 
...
Finally, calculate aggregate county indicators weighing by commodity shares: $p^s(i) = \frac{\sum_c p^s(c, i) d(c, i)}{\sum_c d(c, i)}$ and $p^d(i) = \frac{\sum_c p^d(c, i) s(c, i)}{\sum_c s(c, i)}$.
...
NMS4a = NMS3a %>% 
  ...
  group_by(place) %>% 
  dplyr::summarize(NMS4a = sum(NMS3a*NMS1b)/sum(NMS1b))

NMS4b = NMS3b %>% 
  rename("place"="buyer") %>% 
  left_join(y=NMS1a, by=c("place", "com_code")) %>% 
  group_by(place) %>% 
  dplyr::summarize(NMS4b = sum(NMS3b*NMS1a)/sum(NMS1a))
