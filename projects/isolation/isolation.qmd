...
NMS1a = df_outsupdem %>% 
  dplyr::select(place, com_code, "NMS1a"=supply)

NMS1b = df_outsupdem %>% 
  dplyr::select(place, com_code, "NMS1b"=demand)
...
NMS1c = df_outsupdem %>% 
  ...
  dplyr::select(com_code, "NMS1c"=supply)
...
NMS2a = df_outsupdem %>% 
  ...
  mutate(NMS2a = supply/NMS1c) %>% 
  dplyr::select(place, com_code, NMS2a)

NMS2b = df_outsupdem %>% 
  dplyr::select(place, com_code, demand) %>% 
  left_join(y=NMS1c, by="com_code") %>% 
  mutate(NMS2b = demand/NMS1c) %>% 
