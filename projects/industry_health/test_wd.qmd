...
# library(RColorBrewer)
 ...
 vars = list("paam", "ypll75", "retention_calc", "autonomy_calc", "pct_mfg", "pct_ag", "pct_fire", "pct_edmed", "laborforce_part_rate", "age", "bach", "poverty_rate", "income_rate", "logpop", "uninsured")
 for (v in vars) {
...
viz = rbind(viz_agg, viz_regime) %>% mutate(rurality = ifelse(rurality=="agg","all",rurality))


viz1 = viz %>% filter(var == "retention_calc")
viz2 = viz %>% filter(var == "autonomy_calc")
viz3 = viz %>% filter(var == "pct_ag")

plot1 = ggplot(viz1) +
  geom_errorbar( aes(x=rurality, ymin=coef-se, ymax=coef+se), width=0.4, colour="#3EA5A1", alpha=1.0, size=1.4) +
  ylim(-0.36, 0.51) +
  ...
  ylab("") +
  xlab("") +
  theme(axis.title.y = element_text(size = 10), plot.title = element_text(size = 10, face = "bold"), plot.subtitle = element_text(size = 10))
...
plot3 = ggplot(viz3) +
  geom_errorbar( aes(x=rurality, ymin=coef-se, ymax=coef+se), width=0.4, colour="#306151", alpha=1.0, size=1.4) +
  ylim(-0.36, 0.51) +
...
plot1 + plot2 + plot3
# +
#   plot_annotation(title = "Coefficients",
#           subtitle = "1 SD increase in independent variable on premature death")


viz1 = viz %>% filter(var == "pct_mine")
viz2 = viz %>% filter(var == "pct_mfg")
viz3 = viz %>% filter(var == "laborforce_part_rate")
...
viz1 = viz %>% filter(var == "bach")
viz2 = viz %>% filter(var == "income_rate")
viz3 = viz %>% filter(var == "poverty_rate")

plot1 = ggplot(viz1) +
  geom_errorbar( aes(x=rurality, ymin=coef-se, ymax=coef+se), width=0.4, colour="#306151", alpha=1.0, size=1.4) +
  ylim(-0.36, 0.51) +
  geom_hline(yintercept = 0) +
  ggtitle("% with Bachelor's") +
  theme_classic() +
  ylab("") +
  xlab("") +
  theme(axis.title.y = element_text(size = 10), plot.title = element_text(size = 10, face = "bold"), plot.subtitle = element_text(size = 10))
...
bcoef <- c(NA, NA)
bse <- c(NA, NA)
brurality <- c("metro", "nonmetro")

temp = data.frame(bvar, bcoef, bse, brurality) %>% rename("var"="bvar","coef"="bcoef","se"="bse","rurality"="brurality")
temp2 = viz %>% filter(var == "metro")

temp3 = rbind(temp2, temp)
viz1 = viz %>% filter(var == "age")
viz2 = viz %>% filter(var == "logpop")
viz3 = temp3 %>% filter(var == "metro")
...
viz1 = viz %>% filter(var == "uninsured")
viz2 = viz %>% filter(var == "uninsured")
viz3 = viz %>% filter(var == "uninsured")

plot1 = ggplot(viz1) +
  geom_errorbar( aes(x=rurality, ymin=coef-se, ymax=coef+se), width=0.4, colour="#306151", alpha=1.0, size=1.4) +
  ylim(-1.3, 2) +
  geom_hline(yintercept = 0) +
  ggtitle("% Uninsured", subtitle = "NOTE: different scale") +
  theme_classic() +
  ylab("") +
  xlab("") +
  theme(axis.title.y = element_text(size = 10), plot.title = element_text(size = 10, face = "bold"), plot.subtitle = element_text(size = 10))
...
change y-lims as per approapriate scale
