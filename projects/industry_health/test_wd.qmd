...
circu = circularity %>%  
  filter(year == YR) %>%
  select(county_fips, retention, autonomy) %>% 
...
data$HCaccess_score <- (data$uninsured_norm + data$pcp_norm + data$dentists_norm + data$mhp_norm) / 4
...
```{r}
data %>% select(paam, ypll75, retention, autonomy, emp_to_va, emp_to_output, va_to_output) %>% hist.data.frame()


#density plots
variables_to_plot <- c("paam", "ypll75", "retention", "autonomy", "emp_to_va", "emp_to_output", "va_to_output")
...
ggplot(data, aes(retention, ypll75)) +
  geom_point() +
   geom_smooth(method = "lm") +
   labs(title = "Scatter Plot of Retention and \n Years of Life lost",
        x = "Retention",
...
ggplot(data, aes(autonomy, ypll75)) +
  geom_point() +
   geom_smooth(method = "lm") +
   labs(title = "Scatter Plot of Retention and \n Years of Life Lost",
        x = "Retention",
...
ggplot(nonmetro_data, aes(retention, ypll75)) +
  geom_point() +
   geom_smooth(method = "lm") +
...
ggplot(nonmetro_data, aes(autonomy, ypll75)) +
  geom_point() +
   geom_smooth(method = "lm") +
   labs(title = "Scatter Plot of Autonomy and \n Years of Life Lost for Non-Metro",
        x = "Retention",
