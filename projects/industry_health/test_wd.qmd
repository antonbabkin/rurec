...
ols1 = lm(data = df_map, paam ~ retention_calc + pct_ag + pct_mine + pct_mfg + pct_fire + pct_edmed + laborforce_part_rate + age + highschool_attainment_rate + poverty_rate + income_rate + logpop + HCaccess_score)

ols2 = lm(data = df_map, paam ~ retention_calc + retention_calc*pct_mfg + retention_calc*pct_mine + retention_calc*pct_ag + pct_ag + pct_mine + pct_mfg + pct_fire + pct_edmed + laborforce_part_rate + age + highschool_attainment_rate + poverty_rate + income_rate + logpop + HCaccess_score)
...
## sp autocorrelation
```{r}
mi1 = df_map %>% drop_na(c(paam, retention_calc, pct_mfg, pct_mine, pct_ag, pct_fire, pct_edmed, laborforce_part_rate, age, highschool_attainment_rate, poverty_rate, income_rate, logpop, HCaccess_score))
w <- queen_weights(mi1)
mi1$r = ols1$resid
...
err1 = errorsarlm(data = mi1, paam ~ retention_calc + pct_ag + pct_mine + pct_mfg + pct_fire + pct_edmed + laborforce_part_rate + age + highschool_attainment_rate + poverty_rate + income_rate + logpop + HCaccess_score,
                  lw, zero.policy = TRUE)

err2 = errorsarlm(data = mi1, paam ~ retention_calc + retention_calc*pct_mfg + retention_calc*pct_mine + retention_calc*pct_ag + pct_ag + pct_mine + pct_mfg + pct_fire + pct_edmed + laborforce_part_rate + age + highschool_attainment_rate + poverty_rate + income_rate + logpop + HCaccess_score,
                  lw, zero.policy = TRUE)
...
ols1 = lm(data = mi1, paam ~ retention_calc + pct_ag + pct_mine + pct_mfg + pct_fire + pct_edmed + laborforce_part_rate + age + highschool_attainment_rate + poverty_rate + income_rate + logpop + HCaccess_score)
ols3 = lm(data = mi1, paam ~ autonomy_calc + pct_ag + pct_mine + pct_mfg + pct_fire + pct_edmed + laborforce_part_rate + age + highschool_attainment_rate + poverty_rate + income_rate + logpop + HCaccess_score)
...
```{r}
temp = df_map %>%
  select(paam, retention_calc, pct_mfg, pct_mine, pct_ag, pct_fire, pct_edmed, laborforce_part_rate, age, highschool_attainment_rate, poverty_rate, income_rate, logpop, HCaccess_score) %>%
  st_drop_geometry() %>%
  drop_na()
...
# logpop and HCaccess_score as exogenous covariates
# no interactions


model1 <- '
  retention_calc ~ pct_mfg
  poverty_rate ~ pct_mfg + retention_calc + logpop
  paam ~ logpop + HCaccess_score + poverty_rate + pct_mfg'
...
model2 <- '
  retention_calc ~ pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed
  poverty_rate ~ pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed + retention_calc + logpop
  paam ~ logpop + HCaccess_score + poverty_rate + pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed + retention_calc'
...
model3 <- '
  ...
  retention_calc ~ pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed
  prosperity ~ pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed + retention_calc + logpop
  paam ~ logpop + HCaccess_score + prosperity + pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed + retention_calc
'
...
model4 <- '
  ...
  retention_calc ~ pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed
  prosperity ~ pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed + retention_calc + logpop + retention_calc:pct_mfg + retention_calc:pct_ag
  paam ~ logpop + HCaccess_score + prosperity + pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed + retention_calc
'
...
model5 <- '
  ...
  retention_calc ~ pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed
  prosperity ~ pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed + retention_calc + logpop + retention_calc:pct_mfg + retention_calc:pct_ag
  paam ~ logpop + HCaccess_score + prosperity + pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed + retention_calc + age
'
...
```{r}
mi1 = df_map %>% drop_na(c(paam, retention_calc, pct_mfg, pct_mine, pct_ag, pct_fire, pct_edmed, laborforce_part_rate, age, highschool_attainment_rate, poverty_rate, logpop, HCaccess_score))

sem_pvy = lavPredictY(fit, ynames = c("paam"),
                 xnames = c("pct_mfg", "pct_ag", "pct_mine", "pct_fire", "pct_edmed", "retention_calc", "logpop", "HCaccess_score", "age", "laborforce_part_rate", "poverty_rate", "highschool_attainment_rate"))
...
model6 <- '
  ...
  retention_calc ~ pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed
  prosperity ~ pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed + retention_calc + logpop + retention_calc:pct_mfg + retention_calc:pct_ag
  paam ~ logpop + HCaccess_score + prosperity + pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed + retention_calc + age + splagerr
'
...
model6b <- '
  ...
  retention_calc ~ pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed
  prosperity ~ pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed + a*retention_calc + logpop + retention_calc:pct_mfg + retention_calc:pct_ag
  paam ~ logpop + HCaccess_score + b*prosperity + pct_mfg + pct_ag + pct_mine + pct_fire + pct_edmed + c*retention_calc + age + splagerr
