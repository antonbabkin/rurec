...
mi1 = df_map %>% drop_na(c(paam, retention_calc, pct_nondurable, pct_durable, pct_mine, pct_ag, pct_fire, pct_edmed, laborforce_part_rate, age, bach, poverty_rate, income_rate, logpop, uninsured)) %>% mutate(metro = ifelse(ruc_code >= 4, 1, 0))

ols2a = lm(data = mi1, paam ~ retention_calc + autonomy_calc + pct_ag + pct_mine + pct_mfg + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured)
ols2b = lm(data = mi1, paam ~ retention_calc + pct_ag + pct_mine + pct_mfg + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured)
ols2c = lm(data = mi1, paam ~ retention_calc + pct_ag + pct_mine + pct_mfg + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured)
...
err2a = errorsarlm(data = mi1, paam ~ retention_calc + autonomy_calc + pct_mfg + pct_ag + laborforce_part_rate + pct_mine + age + bach + poverty_rate + income_rate + logpop + metro + uninsured,
                  lw, zero.policy = TRUE)
err2b = errorsarlm(data = mi1, paam ~ retention_calc + pct_mfg + pct_ag + laborforce_part_rate + pct_mine + age + bach + poverty_rate + income_rate + logpop + metro + uninsured,
                  lw, zero.policy = TRUE)
err2c = errorsarlm(data = mi1, paam ~ autonomy_calc + pct_mfg + pct_ag + laborforce_part_rate + pct_mine + age + bach + poverty_rate + income_rate + logpop + metro + uninsured,
                  lw, zero.policy = TRUE)
...
sr2a = errorsarlm(data = sr_data, paam ~ 0 + sr/(retention_calc + autonomy_calc + pct_mfg + pct_ag + laborforce_part_rate + pct_mine + age + bach + poverty_rate + income_rate + logpop + uninsured),
                  lw, zero.policy = TRUE)
sr2b = errorsarlm(data = sr_data, paam ~ 0 + sr/(retention_calc + pct_mfg + pct_ag + laborforce_part_rate + pct_mine + age + bach + poverty_rate + income_rate + logpop + uninsured),
                  lw, zero.policy = TRUE)
sr2c = errorsarlm(data = sr_data, paam ~ 0 + sr/(autonomy_calc + pct_mfg + pct_ag + laborforce_part_rate + pct_mine + age + bach + poverty_rate + income_rate + logpop + uninsured),
                  lw, zero.policy = TRUE)
...
temp = df_map %>%
  mutate(z_lfpr = scale(laborforce_part_rate),
         ...
         z_bach = scale(bach),
         z_inc.rt = scale(income_rate)) %>% mutate(metro = ifelse(ruc_code >= 4, 1, 0))


model5 <- '
  prosperity =~ z_lfpr + z_pov + z_bach + z_inc.rt

  retention_calc ~ pct_mfg + pct_ag + pct_mine
  autonomy_calc ~ pct_mfg + pct_ag + pct_mine
  prosperity ~ pct_mfg + pct_ag + pct_mine + a*retention_calc + x*autonomy_calc + logpop + metro
  paam ~ logpop + metro + uninsured + b*prosperity + pct_mfg + pct_ag + pct_mine + c*retention_calc + z*autonomy_calc + age
...
model5 <- '
  ...
  retention_calc ~ pct_mfg + pct_ag + pct_mine
  prosperity ~ pct_mfg + pct_ag + pct_mine + a*retention_calc + logpop + metro
  paam ~ logpop + metro + uninsured + b*prosperity + pct_mfg + pct_ag + pct_mine + c*retention_calc + age
...
model5 <- '
  ...
  autonomy_calc ~ pct_mfg + pct_ag + pct_mine
  prosperity ~ pct_mfg + pct_ag + pct_mine + x*autonomy_calc + logpop + metro
  paam ~ logpop + metro + uninsured + b*prosperity + pct_mfg + pct_ag + pct_mine + z*autonomy_calc + age
...
<!-- ## Sp Error SEM -->
<!-- ```{r} -->
<!-- mi1 = df_map %>% drop_na(c(paam, retention_calc, pct_durable, pct_nondurable, pct_mine, pct_ag, pct_fire, pct_edmed, laborforce_part_rate, age, bach, poverty_rate, income_rate, logpop, uninsured)) -->

<!-- sem_pvy = lavPredictY(fit, ynames = c("paam"), -->
<!--                  xnames = c("pct_durable", "pct_nondurable", "pct_ag", "retention_calc", "logpop", "uninsured", "age", "z_lfpr", "z_pov", "z_bach", "z_inc.rt")) -->
...
<!--   retention_calc ~ pct_mfg + pct_ag + pct_mine -->
<!--   prosperity ~ pct_mfg + pct_ag + pct_mine + a*retention_calc + logpop -->
<!--   paam ~ logpop + metro + uninsured + b*prosperity + pct_mfg + pct_ag + pct_mine + c*retention_calc + age + splagerr -->
...
ols2a = lm(data = mi1, paam ~ retention_calc + autonomy_calc + retention_calc*pct_mfg + retention_calc*pct_mine + retention_calc*pct_ag + autonomy_calc*pct_mfg + autonomy_calc*pct_mine + autonomy_calc*pct_ag + pct_ag + pct_mine + pct_mfg + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured)

ols2b = lm(data = mi1, paam ~ retention_calc + retention_calc*pct_mfg + retention_calc*pct_mine + retention_calc*pct_ag + pct_ag + pct_mine + pct_mfg + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured)

ols2c = lm(data = mi1, paam ~ autonomy_calc + autonomy_calc*pct_mfg + autonomy_calc*pct_mine + autonomy_calc*pct_ag + pct_ag + pct_mine + pct_mfg + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured)
...
# I had difficulty getting the interacted spatial error models with both retention and autonomy; I addressed this by excluding mining interaction from the model

nb <- poly2nb(mi1, queen=TRUE)
lw <- nb2listw(nb, style="W", zero.policy=TRUE)

err2a = errorsarlm(data = mi1, paam ~ retention_calc + autonomy_calc + pct_mfg + pct_mine + pct_ag + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured + retention_calc*pct_ag + retention_calc*pct_mfg + autonomy_calc*pct_mfg + autonomy_calc*pct_ag,
                  lw, zero.policy = TRUE)

err2b = errorsarlm(data = mi1, paam ~ retention_calc + pct_mfg + pct_mine + pct_ag + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured + retention_calc*pct_mfg + retention_calc*pct_mine + retention_calc*pct_ag,
                  lw, zero.policy = TRUE)

err2c = errorsarlm(data = mi1, paam ~ autonomy_calc + pct_mfg + pct_mine + pct_ag + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured + autonomy_calc*pct_mfg + autonomy_calc*pct_mine + autonomy_calc*pct_ag,
                  lw, zero.policy = TRUE)
...
sr2a = errorsarlm(data = sr_data, paam ~ 0 + sr/(retention_calc + autonomy_calc + pct_mfg + pct_mine + pct_ag + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + uninsured + retention_calc*pct_ag + retention_calc*pct_mfg + autonomy_calc*pct_mfg + autonomy_calc*pct_ag),
                  lw, zero.policy = TRUE)

sr2b = errorsarlm(data = sr_data, paam ~ 0 + sr/(retention_calc + pct_mfg + pct_mine + pct_ag + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + uninsured + retention_calc*pct_mfg + retention_calc*pct_mine + retention_calc*pct_ag),
                  lw, zero.policy = TRUE)

sr2c = errorsarlm(data = sr_data, paam ~ 0 + sr/(autonomy_calc + pct_mfg + pct_mine + pct_ag + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + uninsured + autonomy_calc*pct_mfg + autonomy_calc*pct_mine + autonomy_calc*pct_ag),
                  lw, zero.policy = TRUE)
...
model5 <- '
  ...
  retention_calc ~ pct_mfg + pct_ag + pct_mine
  autonomy_calc ~ pct_mfg + pct_ag + pct_mine
  prosperity ~ pct_mfg + pct_ag + pct_mine + a*retention_calc + x*autonomy_calc + logpop + metro + retention_calc:pct_mfg + retention_calc:pct_ag + autonomy_calc:pct_mfg + autonomy_calc:pct_ag
  paam ~ logpop + metro + uninsured + b*prosperity + pct_mfg + pct_ag + pct_mine + c*retention_calc + z*autonomy_calc + age
...
model5 <- '
  ...
  retention_calc ~ pct_mfg + pct_ag + pct_mine
  prosperity ~ pct_mfg + pct_ag + pct_mine + a*retention_calc + logpop + metro + retention_calc:pct_mfg + retention_calc:pct_ag
  paam ~ logpop + metro + uninsured + b*prosperity + pct_mfg + pct_ag + pct_mine + c*retention_calc + age
...
model5 <- '
  ...
  autonomy_calc ~ pct_mfg + pct_ag + pct_mine
  prosperity ~ pct_mfg + pct_ag + pct_mine + x*autonomy_calc + logpop + metro + autonomy_calc:pct_mfg + autonomy_calc:pct_ag
  paam ~ logpop + metro + uninsured + b*prosperity + pct_mfg + pct_ag + pct_mine + z*autonomy_calc + age
...
<!-- ## Sp Error SEM -->
<!-- ```{r} -->
<!-- mi1 = df_map %>% drop_na(c(paam, retention_calc, pct_nondurable, pct_durable, pct_mine, pct_ag, pct_fire, pct_edmed, laborforce_part_rate, income_rate, age, bach, poverty_rate, logpop, uninsured)) -->
...
<!--   paam ~ logpop + metro + uninsured + b*prosperity + pct_nondurable + pct_durable + pct_ag + pct_mine + c*retention_calc + age + splagerr -->
...
<!-- ols1 = lm(data = df_map, paam ~ retention_calc + pct_ag + pct_mine + pct_mfg + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured) -->

<!-- ols2 = lm(data = df_map, paam ~ retention_calc + retention_calc*pct_mfg + retention_calc*pct_mine + retention_calc*pct_ag + pct_ag + pct_mine + pct_mfg + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured) -->
...
<!-- ## sp autocorrelation -->
<!-- ```{r} -->
<!-- mi1 = df_map %>% drop_na(c(paam, retention_calc, pct_mfg, pct_mine, pct_ag, pct_fire, pct_edmed, laborforce_part_rate, age, bach, poverty_rate, income_rate, logpop, uninsured)) -->
<!-- w <- queen_weights(mi1) -->
<!-- mi1$r = ols1$resid -->
<!-- nb <- poly2nb(mi1, queen=TRUE) -->
<!-- lw <- nb2listw(nb, style="W", zero.policy=TRUE) -->
...
<!-- err1 = errorsarlm(data = mi1, paam ~ retention_calc + pct_ag + pct_mine + pct_mfg + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured, -->
<!--                   lw, zero.policy = TRUE) -->

<!-- err2 = errorsarlm(data = mi1, paam ~ retention_calc + retention_calc*pct_mfg + retention_calc*pct_mine + retention_calc*pct_ag + pct_ag + pct_mine + pct_mfg + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured, -->
<!--                   lw, zero.policy = TRUE) -->
...
<!-- ols1 = lm(data = mi1, paam ~ retention_calc + pct_ag + pct_mine + pct_mfg + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured) -->
<!-- ols3 = lm(data = mi1, paam ~ autonomy_calc + pct_ag + pct_mine + pct_mfg + laborforce_part_rate + age + bach + poverty_rate + income_rate + logpop + metro + uninsured) -->
...
<!-- ```{r} -->
<!-- temp = df_map %>% -->
<!--   select(paam, retention_calc, pct_mfg, pct_mine, pct_ag, pct_fire, pct_edmed, laborforce_part_rate, age, bach, poverty_rate, income_rate, logpop, uninsured) %>% -->
<!--   st_drop_geometry() %>% -->
<!--   drop_na() -->
...
<!--   paam ~ logpop + metro + uninsured + b*poverty_rate + pct_ag + c*retention_calc -->
...
<!--   paam ~ logpop + metro + uninsured + poverty_rate + pct_mfg + pct_ag + pct_mine + retention_calc' -->
...
<!-- model3 <- ' -->
<!--   prosperity =~ laborforce_part_rate + poverty_rate + bach + age -->

<!--   retention_calc ~ pct_mfg + pct_ag + pct_mine -->
<!--   prosperity ~ pct_mfg + pct_ag + pct_mine + retention_calc + logpop -->
<!--   paam ~ logpop + metro + uninsured + prosperity + pct_mfg + pct_ag + pct_mine + retention_calc -->
<!-- ' -->
...
<!-- model5 <- ' -->
<!--   prosperity =~ laborforce_part_rate + poverty_rate + bach -->
...
<!-- ```{r} -->
<!-- mi1 = df_map %>% drop_na(c(paam, retention_calc, pct_mfg, pct_mine, pct_ag, pct_fire, pct_edmed, laborforce_part_rate, age, bach, poverty_rate, logpop, uninsured)) -->

<!-- sem_pvy = lavPredictY(fit, ynames = c("paam"), -->
<!-- <<<<<<< HEAD -->
<!--                  xnames = c("pct_mfg", "pct_ag", "retention_calc", "logpop", "uninsured", "age", "laborforce_part_rate", "poverty_rate", "bach")) -->
<!-- ======= -->
<!-- <<<<<<< HEAD -->
<!--                  xnames = c("pct_mfg", "pct_ag", "retention_calc", "logpop", "uninsured", "age", "laborforce_part_rate", "poverty_rate", "bach")) -->
<!-- ======= -->
<!--                  xnames = c("pct_mfg", "pct_ag", "pct_edmed", "retention_calc", "logpop", "uninsured", "age", "laborforce_part_rate", "poverty_rate", "bach")) -->
<!-- >>>>>>> bf38728ff4223f955701ebe8d7e1b33cb942b384 -->
<!-- >>>>>>> cc1ce359f717e7f0470da833cf96e9811c441b5b -->
...
<!--   retention_calc ~ pct_mfg + pct_ag + pct_mine -->
<!--   prosperity ~ pct_mfg + pct_ag + pct_mine + retention_calc + logpop + retention_calc:pct_mfg + retention_calc:pct_ag -->
<!--   paam ~ logpop + metro + uninsured + prosperity + pct_mfg + pct_ag + pct_mine + retention_calc + age + splagerr -->
<!-- ' -->
...
<!-- sem_pvy = lavPredictY(fit, ynames = c("paam"), -->
<!-- <<<<<<< HEAD -->
<!--                  xnames = c("pct_mfg", "pct_ag", "retention_calc", "logpop", "uninsured", "age", "laborforce_part_rate", "poverty_rate", "bach", "splagerr")) -->
<!-- ======= -->
<!-- <<<<<<< HEAD -->
<!--                  xnames = c("pct_mfg", "pct_ag", "retention_calc", "logpop", "uninsured", "age", "laborforce_part_rate", "poverty_rate", "bach", "splagerr")) -->
<!-- ======= -->
<!--                  xnames = c("pct_mfg", "pct_ag", "pct_edmed", "retention_calc", "logpop", "uninsured", "age", "laborforce_part_rate", "poverty_rate", "bach", "splagerr")) -->
<!-- >>>>>>> bf38728ff4223f955701ebe8d7e1b33cb942b384 -->
<!-- >>>>>>> cc1ce359f717e7f0470da833cf96e9811c441b5b -->
...
<!-- sem_pvy = lavPredictY(fit, ynames = c("laborforce_part_rate", "poverty_rate", "bach"), -->
...
<!-- ni1 = df_map %>% drop_na(c(paam, retention_calc, pct_mfg, pct_mine, pct_ag, pct_fire, pct_edmed, laborforce_part_rate, age, bach, poverty_rate, logpop, uninsured)) -->
...
<!-- sem_pvyz = lavPredictY(fit, ynames = c("laborforce_part_rate", "poverty_rate", "bach"), -->
<!--                  xnames = c("pct_mfg", "pct_ag", "retention_calc", "logpop")) -->
...
<!--   mutate(r_paam = ((1*laborforce_part_rate) - (1*laborforce_part_rate.1) + (-1.035*poverty_rate) - (-1.035*poverty_rate.1) + (0.961*bach) - (0.961*bach.1))) -->
...
<!-- df_map2 = df_map %>% -->
<!--   mutate(prosperity = 1*laborforce_part_rate + -1.035*poverty_rate + 0.961*bach) -->
...
<!-- ols11c = lm(data = df_map2, paam ~ logpop + metro + uninsured + prosperity + pct_mfg + pct_ag + pct_mine + retention_calc + age) -->
...
<!-- ols12c = lm(data = df_map2, paam ~ logpop + metro + uninsured + prosperity + pct_mfg + pct_ag + pct_mine + retention_calc + age) -->
...
<!--                   lw, zero.policy = TRUE) -->
<!-- sem11c = errorsarlm(data = df_map2, paam ~ logpop + metro + uninsured + prosperity + pct_mfg + pct_ag + pct_mine + retention_calc + age, -->
<!--                   lw, zero.policy = TRUE) -->
...
<!--                   lw, zero.policy = TRUE) -->
<!-- sem12c = errorsarlm(data = df_map2, paam ~ logpop + metro + uninsured + prosperity + pct_mfg + pct_ag + pct_mine + retention_calc + age, -->
<!--                   lw, zero.policy = TRUE) -->
...
<!-- df_map3t = df_map %>% -->
<!--   mutate(pros = 1*laborforce_part_rate + -1.035*poverty_rate + 0.953*bach) -->
...
<!--   paam ~ logpop + metro + uninsured + pros + pct_mfg + pct_ag + pct_mine + retention_calc -->
<!-- ' -->
