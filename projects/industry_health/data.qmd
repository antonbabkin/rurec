...
source("projects/industry_health/dataprep.R", local = (dp <- new.env()))
...
# dp$util$zip_pack("tmp/eca_paa_data_v240308.zip", dp$opath)

# unpack all data from a single Zip file
# dp$util$zip_unpack("tmp/eca_paa_data_v240308.zip", overwrite = TRUE)
```
...
`emp_to_va`: Weighted average compensation of employees as share of value added.
`emp_to_output`: Weighted average compensation of employees as share of total output.
`va_to_output`: Weighted average value added as share of total output.
`output_share_ind_X`: Share of industry X in total output.

```{r}
dp$call_industry_structure(2012) |> head()
```
