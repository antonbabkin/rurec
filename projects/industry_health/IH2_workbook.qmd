...
# racial PAAM
```{r}
rpaam_clean = read.csv("data/projects/industry_health/rpaam_output_2025-06-11.csv")
...
summary_stats = data %>% 
  select(paam, log_paam_des, rpaam_AIAN, rpaam_API, rpaam_Black, rpaam_Latine, rpaam_White, retention, rrm_gap, prosperity, pct_mfg, pct_mine, pct_ag, laborforce_part_rate, age, bach, poverty_rate, income_rate, logpop, uninsured) %>%
...
##summary statistics- global
summary_stats = data %>% 
  select(paam, rpaam_AIAN, rpaam_API, rpaam_Black, rpaam_Latine, rpaam_White,retention, rrm_gap, prosperity, pct_mfg, pct_mine, pct_ag, laborforce_part_rate, age, bach, poverty_rate, income_rate, logpop, uninsured) %>%
  get_summary_stats(, type = "common") %>% 
  relocate(variable, n, mean, median, min, max)
summary_stats = as.data.frame(summary_stats)
summary_stats
...
summary_stats_lgmetro = data %>%
  ...
  select(paam, rpaam_AIAN, rpaam_API, rpaam_Black, rpaam_Latine, rpaam_White, retention, rrm_gap, prosperity, pct_mfg, pct_mine, pct_ag, 
...
```{r}
data %>% select(paam, rpaam_AIAN, rpaam_API, rpaam_Black, rpaam_Latine, rpaam_White, retention, rrm_gap, prosperity, pct_mfg, pct_mine, pct_ag) %>% hist.data.frame()
...
#"log_paam_des",
 # Define the variables and titles
vars = c("paam", "rpaam_AIAN", "rpaam_API", "rpaam_Black", "rpaam_Latine", "rpaam_White", "retention", "rrm_gap","rrm_gap_aian", "rrm_gap_asian", "rrm_gap_black", "rrm_gap_hisp", "rrm_gap_white", "rrm_gap_asian_alt", "prosperity", "pct_mfg", "pct_ag", 
           "laborforce_part_rate", "age", "bach", "poverty_rate",  "income_rate", "logpop", "uninsured")

#"Log PAAM",

titles = c("Total Premature\n Death",  "AIAN Premature\n Death", "API Premature\n Death","Black Premature\n Death", "Latine Premature\n Death","White Premature\n Death", "Retention", "Racial Opportunity Gap", "AIAN-White ROG", "API-White ROG", "Black-White ROG", "Latine-White ROG", "White-White OG", "API-API OG", "Prosperity", "%\n Manufacturing",
           "%\n Agriculture", 
           "Labor \nForce\n Participation Rate", "% \nOver 65", 
           "% \nBachelors Plus", "Poverty\n Rate", "Income \nRate", 
           "Log\n Population", "Uninsured \n Rate \nUnder 65")
...
# black_noi = errorsarlm(data = df_black, rpaam ~ 0 + sr3/(retention + rrm_gap + pct_black + pct_hisp + pct_ind + pct_mfg + pct_ag + pct_mine + age + prosperity + logpop + uninsured),
#                   lw_black, zero.policy = TRUE)
# white_noi = errorsarlm(data = df_white, rpaam ~ 0 + sr3/(retention + rrm_gap + pct_black + pct_hisp + pct_ind + pct_mfg + pct_ag + pct_mine + age + prosperity + logpop + uninsured),
#                   lw_white, zero.policy = TRUE)
# aian_noi = errorsarlm(data = df_aian, rpaam ~ 0 + sr3/(retention + rrm_gap + pct_black + pct_hisp + pct_ind + pct_mfg + pct_ag + pct_mine + age + prosperity + logpop + uninsured),
#                   lw_aian, zero.policy = TRUE)
# api_noi = errorsarlm(data = df_api, rpaam ~ 0 + sr3/(retention + rrm_gap + pct_black + pct_hisp + pct_ind + pct_mfg + pct_ag + pct_mine + age + prosperity + logpop + uninsured),
#                   lw_api, zero.policy = TRUE)
# latine_noi = errorsarlm(data = df_latine, rpaam ~ 0 + sr3/(retention + rrm_gap + pct_black + pct_hisp + pct_ind + pct_mfg + pct_ag + pct_mine + age + prosperity + logpop + uninsured),
...
black_i = errorsarlm(data = df_black, rpaam ~ 0 + sr3/(retention + rrm_gap + retention*rrm_gap + pct_black + pct_hisp + pct_ind + pct_mfg + pct_ag + pct_mine + age + prosperity + logpop + uninsured),
                  lw_black, zero.policy = TRUE)
white_i = errorsarlm(data = df_white, rpaam ~ 0 + sr3/(retention + rrm_gap + retention*rrm_gap + pct_black + pct_hisp + pct_ind + pct_mfg + pct_ag + pct_mine + age + prosperity + logpop + uninsured),
                  lw_white, zero.policy = TRUE)
aian_i = errorsarlm(data = df_aian, rpaam ~ 0 + sr3/(retention + rrm_gap + retention*rrm_gap + pct_black + pct_hisp + pct_ind + pct_mfg + pct_ag + pct_mine + age + prosperity + logpop + uninsured),
                  lw_aian, zero.policy = TRUE)
api_i = errorsarlm(data = df_api, rpaam ~ 0 + sr3/(retention + rrm_gap + retention*rrm_gap + pct_black + pct_hisp + pct_ind + pct_mfg + pct_ag + pct_mine + age + prosperity + logpop + uninsured),
                  lw_api, zero.policy = TRUE)
latine_i = errorsarlm(data = df_latine, rpaam ~ 0 + sr3/(retention + rrm_gap + retention*rrm_gap + pct_black + pct_hisp + pct_ind + pct_mfg + pct_ag + pct_mine + age + prosperity + logpop + uninsured),
                  lw_latine, zero.policy = TRUE)
latine_i_top = errorsarlm(data = df_latine_top, rpaam ~ 0 + sr3/(retention + rrm_gap + retention*rrm_gap + pct_black + pct_hisp + pct_ind + pct_mfg + pct_ag + pct_mine + age + prosperity + logpop + uninsured),
                  lw_latine, zero.policy = TRUE)
latine_i_orig = errorsarlm(data = df_latine_orig, rpaam ~ 0 + sr3/(retention + rrm_gap + retention*rrm_gap + pct_black + pct_hisp + pct_ind + pct_mfg + pct_ag + pct_mine + age + prosperity + logpop + uninsured),
                  lw_latine, zero.policy = TRUE)
...
#### ANOVA
```{r}
anova(black_noi, black_i)
anova(white_noi, white_i)
anova(aian_noi, aian_i)
anova(api_noi, api_i)
anova(latine_noi, latine_i)
```
