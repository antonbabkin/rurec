...
paam_plot <- ggplot(data[!is.na(data[["four_metro"]]), ],
  ...
  scale_color_manual(
    ...
    values = c("large_metro" = "#f4b6b6", "large_nonmetro" = "#2c7fb8", 
               "small_metro" = "#8A1B47", "small_nonmetro" = "#08306B"),
    labels = c("Large Metro", "Large Nonmetro", "Small Metro", "Small Nonmetro")
  ) +
  labs(title = "Premature Mortality Rate", x = "Premature Age-Adjusted Mortality", y = "Density") +
  theme_minimal() +
  theme(plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
...
retention_plot <- ggplot(data[!is.na(data[["four_metro"]]), ],
  ...
  labs(title = "Economic Resource Retention", x = "Retention", y = "Density") +
  theme_minimal() +
  theme(
...
rog_plot <- ggplot(data[!is.na(data[["four_metro"]]), ],
  ...
  labs(title = "Racial Opportunity Gap", x = "ROG", y = "Density") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
    legend.position = "left",
...
# Generate paam map
paam_map <- df_map %>%
  tm_shape() + tm_polygons(col = "paam", style = "quantile", n = 4, border.alpha = 0.1, 
      palette = c("#d0e1f2", "#a6c8e1", "#2c7fb8", "#f4b6b6", "#d5303e", "#8A1B47"),
      title = "Deaths <75 \nper 100,000") +
      tm_shape(title_sf) + tm_text("label", size = 1.2, fontface = "bold", just = "center") +
  tm_layout(outer.margins = c(0.01, 0.01, 0.01, 0.01), legend.position = c("left", "center"),  legend.title.size = 1.0, frame = FALSE)
...
retention_map <- df_map %>%
  tm_shape() + tm_polygons(col = "retention", style = "quantile", n = 4, border.alpha = 0.1, 
                           palette = c("#d0e1f2", "#a6c8e1", "#2c7fb8","#f4b6b6", "#d5303e", "#8A1B47"), 
                           title = "Intermediate \nRetention\n Rate") +
  tm_shape(title_sf) + tm_text("label", size = 1.1, fontface = "bold", just = "center") +
  tm_layout(outer.margins = c(0.01, 0.01, 0.01, 0.01), legend.position = c("left", "center"), 
            legend.title.size = 1.0, frame = FALSE)
...
# Generate ROG map
rog_map <- df_map %>%
  tm_shape() + tm_polygons(col = "rrm_gap", style = "quantile", n = 4, border.alpha = 0.1, 
                           palette = c("#d0e1f2", "#a6c8e1", "#2c7fb8","#f4b6b6", "#d5303e", "#8A1B47"), 
                           title = "Rank-rank \nmobility gap") +
  tm_shape(title_sf) + tm_text("label", size = 1.2, fontface = "bold", just = "center") +
  tm_layout(outer.margins = c(0.01, 0.01, 0.01, 0.01), legend.position = c("left", "center"), 
...
# # Get map bounds to place the title
# bbox <- st_bbox(df_map)
# 
# tmap_mode("plot")
# 
...
# # fix retention title
# title_sf <- st_sf(geometry = st_sfc(st_point(c(mean(st_bbox(df_map)[c("xmin", "xmax")]), st_bbox(df_map)["ymax"] + 1))), 
#                   label = "Economic Resource Retention Rate", crs = st_crs(df_map))
# 
# # Generate retention map
# retention_map <- df_map %>%
#   tm_shape() + tm_polygons(col = "retention", style = "quantile", n = 4, border.alpha = 0.1, 
...
# Filter terms and flag significance
results <- results %>%
  filter(grepl("retention|rrm_gap", term), !grepl("prosperity", term)) %>%
  mutate(
    significance = ifelse(p.value < 0.05, "Significant (p < .05)", "Not Significant"),
    Regime = case_when(
      grepl("sr3large\\.metro", term) ~ "Large Metro",
      grepl("sr3small\\.metro", term) ~ "Small Metro",
      grepl("sr3large\\.nonmetro", term) ~ "Large Nonmetro",
...
# Clean and relabel terms
results$term <- gsub("sr3large\\.metro:|sr3small\\.metro:|sr3large\\.nonmetro:|sr3small\\.nonmetro:", "", results$term)
results$term <- recode(results$term,
                       "retention" = "Retention",
                       "rrm_gap" = "ROG",
                       "retention:rrm_gap" = "Retention:ROG")

# Set factor order for top-to-bottom display
results$term <- factor(results$term, levels = rev(c("Retention", "ROG", "Retention:ROG")))
results$Regime <- factor(results$Regime, levels = c("Large Metro", "Small Metro", "Large Nonmetro", "Small Nonmetro"))
...
plot2 <- ggplot(results, aes(x = estimate, y = term, color = significance)) +
  ...
  theme_minimal() +
  scale_color_manual(values = c("Significant (p < .05)" = "#8b0037ff", "Not Significant" = "gray")) +
  labs(
    title = "Retention, ROG, and \nInteraction on Premature Mortality",
    x = "Premature Age-Adjusted Mortality",
    y = "Variable",
    color = "Significance"
  ) +
