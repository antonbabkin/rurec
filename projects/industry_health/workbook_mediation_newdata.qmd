...
summary_stats = data %>% 
  select(paam, retention, autonomy, pct_mfg, pct_mine, pct_ag, laborforce_part_rate, age, bach, poverty_rate, income_rate, logpop, uninsured) %>%
...
summary_stats_metro = data %>%
  ...
  select(paam, retention, autonomy, pct_mfg, pct_mine, pct_ag, 
...
```{r}
data %>% select(paam, retention, autonomy, pct_mfg, pct_mine, pct_ag) %>% hist.data.frame()
...
vars = c("paam", "retention", "autonomy", "pct_mfg", "pct_ag", 
...
# library(RColorBrewer)
 ...
 vars = list("paam", "retention", "autonomy", "pct_mfg", "pct_ag", "pct_fire", "pct_edmed", "laborforce_part_rate", "age", "bach", "poverty_rate", "income_rate", "logpop", "uninsured", "metro_nonmetro")
...
df_map <- left_join(map, data, by = "place") %>% 
  drop_na(paam, ypll75, retention, autonomy, emp_to_va, emp_to_output, va_to_output)
...
ols2a = errorsarlm(data = mi1, prosperity ~ retention + autonomy + pct_ag + pct_mine + pct_mfg + logpop + metro,
                  lw, zero.policy = TRUE)
...
sr1a = errorsarlm(data = sr_data, retention ~ 0 + sr/(pct_ag + pct_mine + pct_mfg + logpop + metro),
                  lw, zero.policy = TRUE)
...
sr2a = errorsarlm(data = sr_data, prosperity ~ 0 + sr/(retention + autonomy + pct_ag + pct_mine + pct_mfg + logpop + metro),
                  lw, zero.policy = TRUE)
...
# tidy_results <- bind_rows(tidy_sr1a, tidy_sr1b, tidy_sr2a, tidy_sr3a)
...
sr1a = errorsarlm(data = sr_data, retention ~ 0 + sr2/(pct_ag + pct_mine + pct_mfg + logpop + metro),
                  lw, zero.policy = TRUE)
...
sr2a = errorsarlm(data = sr_data, prosperity ~ 0 + sr2/(retention + autonomy + pct_ag + pct_mine + pct_mfg + logpop + metro),
                  lw, zero.policy = TRUE)
...
sr1a = errorsarlm(data = sr_data, retention ~ 0 + sr3/(pct_ag + pct_mine + pct_mfg + logpop + metro),
                  lw, zero.policy = TRUE)
sr1b = errorsarlm(data = sr_data, autonomy ~ 0 + sr3/(pct_ag + pct_mine + pct_mfg + logpop + metro),
                  lw, zero.policy = TRUE)
summary(sr1a)
summary(sr1b)
...
sr2a = errorsarlm(data = sr_data, prosperity ~ 0 + sr3/(retention + autonomy + pct_ag + pct_mine + pct_mfg + logpop + metro),
                  lw, zero.policy = TRUE)
