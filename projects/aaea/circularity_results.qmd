...
circu <- circularity$get_circularity()
YR = 2012
df = circu %>%
...
# Clayton - Austin, can you double check my "local satiated intermediates calc"?
...
for (i in indicators) {
  ...
  tm_polygons(col = i, border.alpha = 0.0, style = "cont", id = i,
              palette = c("white","#59ba8c","#235940")) +
  tm_shape(state_map) +
  tm_layout(legend.title.size = 1.0, legend.position = c("LEFT","TOP"), frame = F, fontface = 2) +
  tm_borders(lwd=1.5))
...
for (i in indicators) {
  ...
  print((ggplot(temp, aes(x = .data[[i]], fill = metro_status)) +
           geom_density(alpha = 0.4) +
           theme_pander() +
           scale_fill_pander() +
