...
params = list(
  ...
  control_vars = c("log_pop", "sh_afm", "sh_mfg", "sh_tr", "sh_svc", "sh_gov"),
  rural = "m",
  spreg = FALSE # estimate spatial regression models (slow)
)
...
if (FALSE) {
  ...
  ipath = c(pubdatapy$opath, geography$opath, place_io$opath, cbp$opath)
  # create zip file with cache
  # util$zip_pack("tmp/cache_circ_paper_results_v250115.zip", ipath)
  # unpack cache from zip file
  # util$zip_unpack("tmp/cache_circ_paper_results_v250115.zip", overwrite = TRUE)
}
```
...
for (period in c("base", "end")) {
  ...
  df_busdata[[period]] <- cbp$opath$cbp_ %>%
    glue(geo = "county", cbp_scale = "county", imputed = FALSE) %>%
    open_dataset() %>%
    filter(naics == "") %>%
...
# GDP sector shares
df_sector <- dataprep$gdp_by_sector() %>%
  filter(sector != "tot") %>%
...
df_main <- df_full %>%
  st_drop_geometry() %>%
  select(place, starts_with("rural"), all_of(c(params$outcome_vars, params$circ_indicators, params$control_vars))) %>%
  drop_na()
...
- County population and industry shares are highly statistically significant predictors of circularity indicators.
- Bigger counties have higher circularity and more imports than exports.
- Industry shares:
  - TODO after data revision

```{r}
params$circ_indicators %>%
  set_names(sub("_", " ", .)) %>%
  map(\(x) {
    f = as.formula(paste(x, "~ log_pop + sh_afm + sh_mfg + sh_tr + sh_svc + sh_gov"))
    lm_robust(f, data = df_main)
    }) %>%
...
for (var_out in params$outcome_vars) {
  ...
  msummary(x, output = "huxtable", stars = TRUE) %>%
    set_caption(var_out) %>%
    print()
...
This will be shown later across all models, but this example illustrates the significant effect that population and industry share controls have on results.
With controls, retention effect on employment growth become insignificant, and effect on wage growth changes from negative to positive.


```{r}
x = result_reg %>%
  filter(outcome %in% c("employment_gr", "wage_gr"), circularity == "retention") %>%
...
For reference. See coefficient tables below.
...
result_rureg = expand.grid(
  ...
  pmap(function(...) {
    row = list(...)
    f = paste0(row$outcome, " ~ ", row$circularity, " * rural_", row$rural_def, " + log_pop + sh_afm + sh_mfg + sh_tr + sh_svc + sh_gov")
    m = lm_robust(as.formula(f), data = df_main)
    tibble(..., formula = f, model = list(m))
  }) %>%
  bind_rows()
...
result_intreg = expand_grid(
  ...
  pmap(function(...) {
    ...
    f = paste0(
      ...
      " + log_pop + log_pop + sh_afm + sh_mfg + sh_tr + sh_svc + sh_gov"
    )
    m = lm_robust(as.formula(f), data = df_main)
    tibble(..., formula = f, model = list(m))
  }) %>%
