...
df_main <- df_main |> 
  rural_function(rural_definition)
```
...
print("MAKE A TABLE OF SPATIAL ANALYTSIS STATS")
...
ms <- list()
for (circi in params$circ_indicators) {
  d <- df_main %>%
    st_drop_geometry() %>%
    mutate(circ = .data[[circi]])
  ms[[circi]] <- lm(paste(circi, "~ log_pop + sh_afm + sh_mfg + sh_tr + sh_svc + sh_gov"), data = d) %>%
    summary()
}

huxreg(ms, statistics = c("nobs", "r.squared"))
...
for (circ_measure in params$circ_indicators){
  models= list()
  for (outcome in outcomes){
    formula <- paste(outcome, "~" , circ_measure, " * rural + log_pop + sh_afm + sh_mfg + sh_tr + sh_svc + sh_gov" )
    model_name = gsub("_gr", " growth rate", outcome)  
    models[[model_name]] <- lm(formula, data = df_main)
    # names(models[[model_name]]$coefficients) <- c("constant", 
    #                                               gsub("_", " ", circ_measure),
...
df_main %>%
  ggplot() +
  geom_point(aes(log_pop, retention))
```
...
results_tables$retention_Mmn
results_tables$retention_M
...
results_tables$employment_gr_Mmn
results_tables$employment_gr_M
results_tables$employment_gr_C
