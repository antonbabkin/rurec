...
impute_one <- function(df_1) {
  ...
  df_sol <- df_prior %>%
    select(sp_id, co_id, var, prior = pri) %>%
    inner_join(
      tibble(var = var_sol, sol = sol_qp$solution[1:length(var_sol)]),
      by = "var"
    )
...
df_prep_impute <- df_full %>%
  select(state_alpha, sp_id = stcty, co_id = commodity, measure, value) %>%
  mutate(sp_par = str_ends(sp_id, "000"), co_par = (co_id == "tot"), var = paste0(sp_id, "_", co_id))

states_with_known_totals <- df_prep_impute %>%
  filter(measure == "sale", xor(sp_par, co_par)) %>%
  summarize(known_tot = all(!is.na(value)), .by = state_alpha) %>%
  filter(known_tot) %>%
  pull(state_alpha)
states_with_missing_values <- df_prep_impute %>%
  filter(measure == "sale") %>%
  summarize(mis = any(is.na(value)), .by = state_alpha) %>%
  filter(mis) %>%
  pull(state_alpha)
states_to_impute <- base::intersect(states_with_missing_values, states_with_known_totals)

df_imputations <- list()
for (i_st in states_to_impute) {
  print(i_st)
  df_sol <- impute_one(df_prep_impute %>% filter(state_alpha == i_st))$df
  df_imputations[[i_st]] <- df_sol
}
df_imputations <- df_imputations %>%
  bind_rows(.id = "state_alpha") %>%
  separate_wider_delim(var, "_", names = c("stcty", "name")) %>%
  mutate(value_imp = round(sol), name = paste0("sale_", name)) %>%
  select(stcty, name, value_imp)

df_imputed <- df_full %>%
  left_join(df_imputations, by = c("stcty", "name"))
...
agcensus_dataset(2012) %>%
  filter(str_detect(short_desc, "FIELD .*OPER.*SALE")) %>%
  distinct(short_desc) %>%
  collect()
...
```{r}
agcensus_dataset(2012) %>%
  filter(str_detect(short_desc, "GRAIN.*SALE.*")) %>%
  filter(agg_level_desc %in% c("NATIONAL", "STATE", "COUNTY"), domain_desc == "TOTAL", state_fips_code == "10", is.na(county_code)) %>%
  select(short_desc, value) %>%
  collect()
```
...
```{r}
x = farm_sales() %>%
  filter(id_sp1 == "29", id_sp2 == "000")
x %>%
  filter(measure == "nsal") %>%
  select(commodity, value, short_desc) %>% 
  arrange(commodity)
```
...
Fishing for a nice subset for demonstration.
...
solv <- sol$solution[1:nv] %>%
  `names<-`(nmv)
sols <- sol$solution[nv + 1:nc] %>%
  `names<-`(nms)
...
Slack. A handful of constraints are satisfied with significant slack.

```{r}
sols[sols > 5] %>% round(1) %>% sort()
```


Why zero sometimes? Because same variable is in multiple constraints, and relative deviations are balanced between them.

```{r}

df_src %>%
  filter(str_detect(id, "50_009_1c_5fr_....")) %>%
  pivot_wider(id_cols = id, names_from = measure)

df_con %>%
  filter(id == "50_009_1c_5fr_1tre")


df_con %>%
  filter(id == "01_001_2a_1po_0000") %>%
  pull(id_par) %>%
  {filter(df_con, id_par %in% .)} %>%
  arrange(id_par, id)
