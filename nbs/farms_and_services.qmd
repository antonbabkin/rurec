...
d <- df %>%
  ...
  mutate(agser_disc = factor(agser_disc, c("zero", "suppressed", "available"))) %>%
  mutate(sales_crop_pct = farm_sales_crop / farm_sales_tot)

m <- list()
# m[["plain"]] <- lm(agser_pay ~ farm_sales_tot, data = d)
m[["wgt"]] <- lm(agser_pay ~ farm_sales_tot, data = d, weights = farm_sales_tot)
...
m[["log + wgt"]] <- lm(log(agser_pay) ~ log(farm_sales_tot), data = d, weights = farm_sales_tot)
m[["log + wgt + year"]] <- lm(log(agser_pay) ~ log(farm_sales_tot) + factor(year), data = d, weights = farm_sales_tot)
m[["log + wgt + year + crop"]] <- lm(log(agser_pay) ~ log(farm_sales_tot) * sales_crop_pct + factor(year), data = d, weights = farm_sales_tot)
m[["log + wgt + year + hhi"]] <- lm(log(agser_pay) ~ log(farm_sales_tot) * farm_sales_tot_hhi + factor(year), data = d, weights = farm_sales_tot)
m[["log + wgt + year + crop + hhi"]] <- lm(log(agser_pay) ~ log(farm_sales_tot) * (sales_crop_pct + farm_sales_tot_hhi) + factor(year), data = d, weights = farm_sales_tot)

x <- m[["log + wgt + year"]]$coefficients %>% names()
omit <- str_subset(x, "year")
huxreg(m, omit_coefs = omit, statistics = c(N = "nobs", R2 = "r.squared"))
```
