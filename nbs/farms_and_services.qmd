...
run_tagged_chunks <- function(tag) {
  # start with clean knitr session
  knitr::knit_code$restore()
  # parse and load chunks from active notebook into knitr session
  src <- rstudioapi::getActiveDocumentContext()$content
  chunks <- knitr:::split_file(src, patterns = knitr::all_patterns$md)
  # split_file returns chunks metadata, and code can be accessed with knit_code$get
  for (c in chunks) {
    if (!is.null(c$params) && !is.null(c$params$tags) && (tag %in% c$params$tags)) {
      print(c("---- run chunk:", c$params$label))
      code <- knitr::knit_code$get(c$params$label)
      if (!is.null(c$params$engine) && (c$params$engine == "python"))
        rstudioapi::sendToConsole(c("repl_python(quiet = TRUE)", code, "exit"))
      else
...
```{r}
run_tagged_chunks("def-data-prep")
```
...
In 2012, estabs in suppressed cells are reported, and only emp and pay are zero.
...
def prep_data_farm_count():
  ...
  df = agcensus.get_df(PAR['years'], ['YEAR', 'STATE_FIPS_CODE', 'COUNTY_CODE', 'SHORT_DESC', 'VALUE', 'VALUE_F'],
                       [
                         ('AGG_LEVEL_DESC', '==', 'COUNTY'),
                         ('DOMAIN_DESC', '==', 'TOTAL'),
...
def prep_data_farm_conc_land():
  ...
  df = agcensus.get_df(PAR['years'], ['YEAR', 'STATE_FIPS_CODE', 'COUNTY_CODE', 'SHORT_DESC', 'DOMAINCAT_DESC', 'VALUE', 'VALUE_F'],
                       [('AGG_LEVEL_DESC', '==', 'COUNTY'),
...
def prep_data_agser():
  df = qcew.get_df(PAR['years'], ['year', 'area_fips'], [('agglvl_code', '==', '70')])
  
  d = qcew.get_df(PAR['years'], 
...
def prep_data_county_other():
  df = qcew.get_df(PAR['years'], ['year', 'area_fips', 'total_annual_wages'], [('agglvl_code', '==', '70')])
  df = df.rename(columns={'area_fips': 'stcty', 'total_annual_wages': 'pay'})
  df['pay'] = nom2real(df['year'], df['pay'])
  
  d = population.get_df()
  d = d[d['year'].isin(PAR['years']) & (d['cty'] != '000')].copy()
  d['stcty'] = d['st'] + d['cty']
  d = d[['stcty', 'year', 'pop']]
...
- Positive association between farms and ag services size.
- Counties with zero or suppressed ag services have smaller farming.
- Farm sales in zero are less than in suppressed.
...
All regressions only include counties where agser payroll is available (exclude zero and suppressed).

Effects of log-scale and weighting.

Because county size distribution is strongly right-skewed, estimates in levels can be strongly affected by outliers.
...
```
To summarize, in the sample of counties with positive "NAICS: 115" payroll (about 30% of all counties), size of the ag services sector is positively associated with the size of the farming sector.
On average, 1% higher farm sales correspond to 0.4% more in ag services payroll.
The relationship stronger in counties that are bigger, are more crop oriented, and have higher market concentration in farm sales.
...
One simple way to measure crop farming productivity is to divide sales by land.

```{r}
d <- df %>%
  filter(agser_disc1151 == "available") %>%
  mutate(farm_crop_prod = farm_sales_crop / farm_land_crop) %>%
  mutate(farm_crop_prod = if_else(farm_crop_prod < 10000, farm_crop_prod, NA_real_)) %>% # outliers
  mutate(log_farm_crop_prod = log(farm_crop_prod))

ggplot(d, aes(log_farm_crop_prod)) +
  geom_density(aes(color = factor(year))) +
  scale_x_continuous("farm_crop_prod, $/acre", labels = function(x) format(exp(x), digits=2, scientific = TRUE)) 
```


```{r}
pairs(~ log(cty_pay) + log(agser_pay1151) + log(farm_sales_crop) + log(farm_land_crop) + log(farm_crop_prod), d)
```
...
d <- df %>%
  ...
  mutate(farm_crop_prod = farm_sales_crop / farm_land_crop) %>%
  mutate(farm_crop_prod = if_else(farm_crop_prod < 10000, farm_crop_prod, NA_real_)) %>% # outliers
  mutate(across(c(agser_pay1151, farm_sales_crop, cty_pay, farm_land_crop, farm_crop_prod), log, .names = "log_{.col}"))

m <- lm(log_farm_crop_prod ~ log_agser_pay1151 * (log_cty_pay + log_farm_sales_crop) + factor(year), d, weights = log_agser_pay1151)
huxreg(m)
```
...
ggplot(mapping = aes(x = at_q)) +
  ...
  scale_color_manual("interaction variable", values = colors) + scale_fill_manual(values = colors, guide = "none") +
  scale_x_continuous("interaction variable quantile") + scale_y_continuous("elasticity") +
  labs(title = "log_farm_crop_prod ~ log_agser_pay1151 * (log_cty_pay + log_farm_sales_crop)")
...
p +
  ...
  labs(title = "farm_corn_yield ~ log_agser_pay1151 * (log_farm_sales_crop + log_farm_corn_land)") + 
...
p +
  ...
  scale_color_manual("interaction variable", values = colors) + scale_fill_manual(values = colors, guide = "none") +
  scale_x_continuous("interaction variable quantile") + scale_y_continuous("marginal effect") +
  labs(title = "farm_soy_yield ~ log_agser_pay1151 * (log_farm_sales_crop + log_farm_soy_land)") + 
  theme(plot.title = element_text(size = 10))
```
