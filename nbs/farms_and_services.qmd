...
```{r}
t <- py$t_qcew_county_suppression %>%
...
df <- df %>%
  group_by(stcty) %>%
  arrange(stcty, year) %>%
  mutate(across(starts_with(c("agser", "farm")) & !agser_disc, growth_rate, .names = "{.col}_gr"))
...
ggplot(mapping = aes(x = at_q)) +
  ...
  scale_color_manual("interaction variable", values = colors) + scale_fill_manual(values = colors, guide = FALSE) +
