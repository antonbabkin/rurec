...
if (params$map_tradeflow_show) {
  ...
  trade_flows$call_trade_flows_custom(industry_code) %>%
    {connectedness$eca_table_central(focus_county, .)} %>% 
    {inner_join(geog$call_geog(), ., by = "place")} %>% 
    {visual$place_trade_map(place_trade_table = ., 
                            central_place = focus_county, 
                            fill_variable = "export_absorption", 
                            censor_scale_lowerbound = min(.[["export_absorption"]][.[["export_absorption"]] > 0]) )}
  ...
  trade_flows$call_trade_flows_custom(industry_code) %>%
    ...
    {visual$place_trade_map(place_trade_table = ., 
                            ...
                            censor_scale_lowerbound = min(.[["import_absorption"]][.[["import_absorption"]] > 0]) )}
