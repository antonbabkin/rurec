...
params:
  ...
  map_tradeflow_show: true
  map_tradeflow_county: "55025"
  map_tradeflow_ind: "all_industries" # "all_industries", single code ("1111A0"), comma-separated list ("1111A0,713900") or regex ("^112.*$")
...
if (params$map_tradeflow_show) {
  ...
  p1 <- tm_shape(filter(df, contiguous)) + tm_fill("imports") +
  ...
  p2 <- tm_shape(filter(df, contiguous)) + tm_fill("exports") +
    tm_shape(filter(df, place == focus_county)) + tm_fill("blue") +
    tm_shape(filter(geo$pubdata$get_state_df(TRUE), CONTIGUOUS)) + tm_borders() +
...
source("R/visualization.R", local = (visual <- new.env()))
  ...
  industry_code <- "112120"
  focus_county <- "55117"
  fname <- paste0(industry_code, ": Import Flows")
...
names(dft)[names(dft) == "import_absorption"] <- fname

 dft %>%  {visual$place_trade_map(place_trade_table = ., 
                            central_place = focus_county, 
                            fill_variable = fname, 
                            censor_scale_lowerbound = 10000,
                            interactive = F)}
...
if (params$map_tradeflow_show) {
  ...
  trade_flows$call_trade_flows_custom(industry_code) %>%
    {connect$eca_table_central(focus_county, .)} %>% 
    {left_join(geo$call_geog(), ., by = "place")} %>% 
    {visual$place_trade_map(place_trade_table = ., 
                            central_place = focus_county, 
                            fill_variable = "export_absorption", 
                            censor_scale_lowerbound = min(.[["export_absorption"]][.[["export_absorption"]] > 0], na.rm = T) )}
  ...
  trade_flows$call_trade_flows_custom(industry_code) %>%
    ...
    {visual$place_trade_map(place_trade_table = ., 
                            central_place = focus_county, 
                            fill_variable = "import_absorption", 
                            censor_scale_lowerbound = min(.[["import_absorption"]][.[["import_absorption"]] > 0], na.rm = T) )}
...
if (params$map_tradeflow_show) {
  ...
  trade_flows$call_trade_flows_custom(industry_code) %>%
    {connect$eca_table_central(focus_county, .)} %>% 
    {inner_join(geo$call_geog(), ., by = "place")} %>% 
    {visual$place_trade_map(place_trade_table = ., 
                            central_place = focus_county, 
                            fill_variable = "export_absorption", 
                            censor_scale_lowerbound = min(.[["export_absorption"]][.[["export_absorption"]] > 0], na.rm = T) )}
...
if (params$sector_hist_plot) {
  ...
  key = list(
    ...
    cbsa = FALSE,
    central_place = 55117, # "01001"
    trim = "^(60|66|69|72|78)|(999)$",
    cluster_subset = "^(112120|311513|311410)", # "^11"
    color = "turbo", # c("turbo", "viridis", "magma", "inferno", "plasma", "cividis", "rocket", "mako", "rainbow")
    cluster_level = "det", # c("sec", "sum", "det")
    verbose = FALSE)
  
   tmp <-  visual$call_sector_hist_table(
    year = key$year,
    ilevel = key$ilevel,
    class_system = key$class_system,
    paradigm = key$paradigm,
...
if (params$sector_hist_plot) {
  ...
  key = list(
    ...
    trim = "^(60|66|69|72|78)|(999)$",
    cluster_subset = "^(112120|311513|311410)", # "^11"
    indicator = "net_demand", # c("gross_output", "intermediate_supply", "intermediate_demand", "net_supply", "net_demand")
    color = "turbo", # c("turbo", "viridis", "magma", "inferno", "plasma", "cividis", "rocket", "mako", "rainbow")
    cluster_level = "det", # c("sec", "sum", "det")
    interactive = F)
  
  hist <- visual$call_sector_histogram(
    year = key$year,
    ilevel = key$ilevel,
    class_system = key$class_system,
    paradigm = key$paradigm,
...
if (params$sector_hist_plot) {
  ...
  key = list(
    ...
    trim = "^(60|66|69|72|78)|(999)$",
    cluster_subset = NULL, # "^11"
    indicator = "net_supply", # c("gross_output", "intermediate_supply", "intermediate_demand", "net_supply", "net_demand")
    color = "turbo", # c("turbo", "viridis", "magma", "inferno", "plasma", "cividis", "rocket", "mako", "rainbow")
    cluster_level = "sum", # c("sec", "sum", "det")
    interactive = F)
...
if (params$eca_map) {
  ...
  conn_metrics <- connect$apply_absorption_metrics(tf_norm)
  d <- connect$apply_absorption_algorithm(conn_metrics, threshold = 0)
  d1 <- right_join(geo$call_tiger(), d, "place")
  
  visual$cluster_map(
    spatial_dataframe = d1,
    fill_variable = "eca_membership",
    caption = "ECA from trade flows", 
    interactive = TRUE)
}
...
#| warning: false
# TODO: add relevant data to ipath
if (params$circularity) {
  # explore circularity metrics 
 ...
 df <- circ$call_circularity_metrics(
    year = key$year,
    ilevel = key$ilevel,
    class_system = key$class_system,
    paradigm = key$paradigm,
...
df %>% {st_drop_geometry(.[,8:21])} %>% cor(use = "complete.obs")
}
...
#| warning: false
# TODO: add relevant data to ipath
if (params$econ_dynam_data) {
  #explore economic dynamism data
   
  key = list(
    year = 2012
    )
  
   df <- dataprep$call_econ_dynam_ind(
          year = key$year) %>% 
     {left_join(geo$call_geog(year = key$year), ., by = "place")} 
...
# TODO: add relevant data to ipath
if (params$econ_dynam_data) {
  #map economic dynamism levels
  
  key = list(
    year = 2012,
    fill_variable = "employment" # c("population", "employment", "establishments", "output", "entry", "exit", "entry_rate", "exit_rate", "population_grow_rate", "employment_grow_rate", "establishments_grow_rate", "output_grow_rate")   
  )
  ...
  visual$nominal_choro_map(df, fill_variable = key$fill_variable)
...
# TODO: add relevant data to ipath
if (params$econ_dynam_data) {
  #map economic dynamism growth rates
      ihs_trans <- function(){trans_new("ihs", function(x){asinh(x)}, function(x){sinh(x)} )}
      
  key = list(
    year = 2012,
    fill_variable = "employment_grow_rate" # c("population", "employment", "establishments", "output", "entry", "exit", "entry_rate", "exit_rate", "population_grow_rate", "employment_grow_rate", "establishments_grow_rate", "output_grow_rate")   
  )
  ...
  visual$diverge_choro_map(df, fill_variable = key$fill_variable)
