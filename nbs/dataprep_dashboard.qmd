...
source("rurec/reseng.R", local = (reseng <- new.env()))
...
paths <- names(dw$paths) %>%
  keep(\(x) str_starts(x, "dashboard_")) %>%
  rlang::set_names() %>%
  map(\(x) dw$paths[[x]]) %>%
  rlang::set_names(str_remove(names(.), "dashboard_"))
paths
...
dw$pack("tmp/data_dashboard_v231929.zip", paths)
```
...
write_rds(df, dw$mkdir(paths$county_geography), compress = "gz", version = 3)
```
...
```{r}
df <- read_rds(paths$county_geography) %>%
  as_tibble() %>%
  select(stcty_fips)
...
write_rds(df, dw$mkdir(paths$county_agg_stats), compress = "gz", version = 3)
```
...
write_rds(df, dw$mkdir(paths$county_ind_outsupdem), compress = "gz", version = 3)
```
