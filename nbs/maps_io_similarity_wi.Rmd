...
Similarly, let the *share* or *relative input needs* vector, denoted with a tilde, be given as $\mathbf{\widetilde{Ax^{r}}} = \mathbf{Ax^{r}}(\mathbf{iAx^{r}})^{-1}$. 
...
"Queeg distance": $\mathbf{i'} \min \{\max \{\mathbf{x^{r}} - \mathbf{Ax^{r}}, 0\}, \max \{\mathbf{Ax^{s}} - \mathbf{x^{s}}, 0\} \} ~ \forall ~ r,s \in l$

```{r}
WI_Test_Plots_XINT[[7]]
```
...
"Spatial Queeg distance": $\mathbf{i'} \min \{\max \{\mathbf{x^{r}} - \mathbf{Ax^{r}}, 0\}, \max \{\mathbf{Ax^{s}} - \mathbf{x^{s}}, 0\} \} / Q^{rs} ~ \forall ~ r,s \in l$

```{r}
WI_Test_Plots_Imped_XINT[[7]]
```
...
Let $Q^{rs} = 2/(e^{-d^{rs}c} + e^{d^{rs}c})$ and scalar $c = 10^{6}$ 
...
Sim <- Sim_list[[1]][[1]][rural, primary]
...
ggplot(  filter(TIGER_RUCC_h1m, STATE_CODE=="55")) +
  ...
  labs(title = "              First Order Similarity Matching",
       ...
       scale_fill_discrete(labels = (TIGER_RUCC_h1m  %>% filter(place %in% unique(TIGER_RUCC_h1m$match) ) %>% pull(COUNTY)) )
