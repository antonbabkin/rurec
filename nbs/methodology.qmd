...
wi_counties <- str_starts(rownames(x), "55")
plot_heatmap(x[wi_counties, wi_counties]) + labs(title = "Total trade flows in Wisconsin")

x1 <- sweep(x, 1, rowSums(x), "/")
x1[is.na(x1)] <- 0
x2 <- conn$apply_absorption_metrics(x1)
x3 <- conn$apply_absorption_algorithm(x2)
x3 |> select(place, cluster_category, eca_membership)
