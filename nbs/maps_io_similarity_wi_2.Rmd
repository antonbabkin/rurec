     ...
     ggbase[[l]][[p]] <- ggplot(as.data.frame(test_industry[[l]][[p]])) +
      ...
      scale_y_continuous(trans='log10') +
       labs(title = "Payroll Distribution by Industry",
           color = "Counties") + 
       scale_color_discrete( labels = (TIGER_RUCC %>% filter(TIGER_RUCC$FIPS %in% primary_extent) %>% pull(County_Name)) )
    }
}
...
"Spatial Impedance Only": $1 / Q^{rs} ~ \forall ~ r,s \in l$

```{r}
  mapr(WI_TT, impedance = Impede_mat[[1]], spo = TRUE)
  WI_TT[[1]][[1]] 
  rm(WI_TT)
```
...
if (!file.exists(file.path(data_dir, "WI_Test_Sens_XINT"))){
    ...
    WI_Test_Sens_XINT <- list()
  
    Imp <- vector(mode='list', length = 10)
    ...
    x=x+1
    for (i in 1:length(DI_mat)){ Imp[[x]][[i]] = ((1/(DI_mat[[i]])^2) * 1000) } 
    x=x+1
    for (i in 1:length(DI_mat)){ Imp[[x]][[i]] = ((1/(DI_mat[[i]])^2) * 10000000) } 
    x=x+1
    for (i in 1:length(DI_mat)){ Imp[[x]][[i]] = exp(-(DI_mat[[i]])) }
    x=x+1
    for (i in 1:length(DI_mat)){ Imp[[x]][[i]] = exp(-(DI_mat[[i]]/10)) }
    x=x+1
    for (i in 1:length(DI_mat)){ Imp[[x]][[i]] = exp(-(DI_mat[[i]]/10000)) }
    x=x+1
    for (i in 1:length(DI_mat)){ Imp[[x]][[i]] = exp(-(DI_mat[[i]]/1000000)) }
    x=x+1
    for (i in 1:length(DI_mat)){ Imp[[x]][[i]] = ((2/(exp(-(D_mat[[i]]/1000000)) + exp(D_mat[[i]]/1000000)))/10) }
...
Specification sensitivity example for various impedance measures using the "Relative Import Similarity Index - Net Exports" data.  
...
$Q^{rs} = (1/(d^{rs})^2)c$ where scalar $c = 10,000,000$ 
...
$Q^{rs} = e^{-d^{rs}c}$ where scalar $c = 100,000$  100000
...
$Q^{rs} = (2/(e^{-d^{rs}c} + e^{d^{rs}c}))k$ where scalar $c = 1,000,000$ and scalar $k = 10$
...
primary_extent <- c(grep("^55", rownames(TIGER_RUCC), value=TRUE) , "17031", "27053")

## Alternative "natural" specification with no predefined urban hubs
# rural_extent <- c(grep("^55", rownames(TIGER_RUCC), value=TRUE) , "17031", "27053")
# primary_extent <- c(grep("^55", rownames(TIGER_RUCC), value=TRUE) , "17031", "27053")
...
if (!file.exists(file.path(data_dir, "WI_Test_Plots_Nat_XINT"))){
  ...
  saver(WI_Test_Plots_Nat_XINT)
} else {
  importr(WI_Test_Plots_Nat_XINT)
}
...
if (!file.exists(file.path(data_dir, "WI_Test_Plots_Nat_Imped_XINT"))){
  if (!exists("WI_Test_Plots_Nat_Imped")){
    bmapr(WI_Test_Plots_Nat_Imped, impedance = Impede_mat[[3]])
  }
  if (!exists("WI_Test_Plots_Nat_Imped_XINT")){
    dismapr(WI_Test_Plots_Nat_Imped)
  }
  saver(WI_Test_Plots_Nat_Imped_XINT)
} else {
  importr(WI_Test_Plots_Nat_Imped_XINT)
}
...
rural <- rownames(Impede_mat[[1]][[1]])[rownames(Impede_mat[[1]][[1]]) %in% filter(TIGER_RUCC, H3==3)$place ]
primary <- rownames(Impede_mat[[1]][[1]])[rownames(Impede_mat[[1]][[1]]) %in% filter(TIGER_RUCC, H3==2)$place ]
