...
```{r}
mapsim <- function(similarity, industry){ 
  
  dataset <- Sim_list[[similarity]][[industry]][WItest_rural, WItest_primary]

  mincom <- cbind(place = rownames(dataset), match = colnames(dataset)[apply(dataset, 1, which.min)], min_value = apply(dataset, 1, min)) %>% as.data.frame()
  mincom$min_value <- as.numeric(mincom$min_value)
  mincom %<>% group_by(match) %>% mutate(Nor = min_value/max(min_value)) %>% as.data.frame()
  mincom <- rbind(  mincom, as.data.frame(cbind(place = WItest_primary, match = WItest_primary, min_value = diag(Sim_list[[similarity]][[industry]][WItest_primary, WItest_primary]), Nor = rep(c(1), each=length(WItest_primary)) )))
  mincom$Nor <- as.numeric(mincom$Nor)
  
  
  fdataset <- inner_join(TIGER_RUCC, mincom, by = "place", copy = TRUE)
  fdataset %<>% mutate(match_name = fdataset$NAME[match(match, fdataset$place)])
  
  
ggplot( fdataset ) +
    geom_sf(aes(fill = match, alpha = Nor, tooltip = glue("County: {NAME}\nFIPS: {place}\nMatch: {match_name}"), data_id = place), color = NA) +
    guides(alpha = "none") +
    coord_sf() +
    theme_void() +
    labs(fill = "Regional Cluster") +
    scale_fill_brewer(palette = "Set1",
                      labels = (fdataset  %>% filter(place %in% unique(fdataset$match) ) %>% pull(County_Name)) ) 
...
df <- function(similarity, industry){ 
...
#mapsim("Sim_mat", "Sector") 


manipulateWidget(
  mapsim(similarity, industry),
  similarity = mwSelect(c("Sim_mat", "Sim_mat_rel",  "Sim_mat_imp", "Sim_mat_exp", "Sim_mat_imp_rel", "Sim_mat_exp_rel")),
  industry = mwSelect(c("Sector", "Summary", "Detail"))
)
