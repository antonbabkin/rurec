...
params:
  ...
  reg_factors: !expr c("gross_output", "population", "laborforce_part_rate", "unemployment_rate", "employment_rate", "entry_rate", "exit_rate")
...
df_geo <- geog$call_geog(year = params$year) %>% 
  {left_join(., geog$call_cbsa_concord(params$year), by = "place")} %>% 
  mutate(CBSA_CODE = ifelse(is.na(CBSA_CODE), "rural", CBSA_CODE)) %>%
  mutate(CBSA_TITLE = ifelse(is.na(CBSA_TITLE), "rural", CBSA_TITLE))

df_geo <- geog$pubdata$get_cbsa_delin_df(year = util$year2cbsa(params$year)) %>% 
  mutate(place = {paste0(.$STATE_CODE, .$COUNTY_CODE)}) %>% 
  {.[c("place", "CENTRAL_OUTLYING")]} %>% 
  {left_join(df_geo, ., by = "place")} %>% 
  mutate(CENTRAL_OUTLYING = ifelse(is.na(CENTRAL_OUTLYING), "rural", CENTRAL_OUTLYING))


df_production <- place_output$call_extraction_table(year = params$year, bus_data = params$bus_data, class_system = params$class_system, ilevel = "det") %>% 
  st_drop_geometry() %>%
  {.[c("place", "gross_output", "intermediate_supply", "intermediate_demand", "net_supply", "net_demand")]}

dl <- list(
  dataprep$call_county_population(year = params$year, bus_data = "census"),
...
df <- inner_join(df_geo, df_production,  by = "place") %>% 
...
eca_data <- dp$call_eca_df() %>% rename(place = fips)

df <- inner_join(df, eca_data,  by = "place")
...
for (i in params$reg_factors ){
  specif[[i]] <- df %>% 
    {.[.$eca_central_out_xtab == "Outlying Source", ] } %>% 
...
specif <- list()
for (i in params$reg_factors){
   specif[[i]] <- df[df$eca_central_out_xtab == "Outlying Source", ] %>% 
