...
# f <- file.path(find_rstudio_root_file(), "data", "robjs", "trade_base", "tester") %>% list.files() %>% grep("^11", . , value = T) 
# fl <- lapply(file.path(find_rstudio_root_file(), "data", "robjs", "trade_base", "tester", f), readRDS)
# df <- Reduce('+', fl)
# df2 <- df[!grepl('^(02|15|72)', rownames(df)), !grepl('^(02|15|72)', colnames(df))] 

```

```{r}

temp <- lapply(file.path(find_rstudio_root_file(), "data", "robjs", "trade_base", "tester", "1111B0"), readRDS) %>% .[[1]]
temp <- temp[!grepl('^(02|15|72)', rownames(temp)), 
             !grepl('^(02|15|72)', colnames(temp))]
temp %>% sna_value_map(metric = "eigen_centrality")

test <- temp %>% graph_from_adjacency_matrix(weighted = TRUE, mode = "directed") %>% eigen_centrality(., directed = TRUE, scale = F) %>% .[[1]]
```

```{r}
df2 %>% sna_value_map(metric = "eigen_centrality")
```

```{r}
df %>% sna_value_map(metric = "alpha_centrality")
```

```{r}
df2 %>% sna_value_map(metric = "closeness")
```

```{r}
df2 %>% sna_value_map(metric = "harmonic_centrality")
```

```{r}
df2 %>% sna_value_map(metric = "hub_score")
```

```{r}
df2 %>% sna_value_map(metric = "authority_score")
```

```{r}
df2 %>% sna_value_map(metric = "page_rank")
```

```{r}
df2 %>% sna_value_map(metric = "strength")
```
...
industryflow = TRUE
impedance_mat = NULL
#unique(call_ilevel_concord(ilevel)[1])
subsectors = NULL
impedance_call = NULL
crosshaul = TRUE
...
#                                 industryflow = T,
#                                 impedance_mat = NULL,
#                                 subsectors = unique(call_ilevel_concord("det")[1])[1:13,],
#                                 verbose = TRUE)
# 
