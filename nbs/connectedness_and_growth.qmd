...
.county_farm_emp <- function() {
  ...
  df <- ds %>%
    ...
    collect() %>%
    mutate(short_desc = case_when(
      short_desc == "LABOR, HIRED - OPERATIONS WITH WORKERS" ~ "est_farm",
      short_desc == "LABOR, HIRED - NUMBER OF WORKERS" ~ "emp_farm"
    )) %>% 
  ...
  df <- df %>% 
    group_by(stcty) %>% 
    mutate(
      emp_farm = round(zoo::na.approx(emp_farm, year, na.rm = FALSE, rule = 2)),
      est_farm = round(zoo::na.approx(est_farm, year, na.rm = FALSE, rule = 2)))
  
  return(df)
}
# file.remove(file.path(root_dir, "cache/county_farm_emp.rds"))
county_farm_emp <- reseng$simplecache(.county_farm_emp, file.path(root_dir, "cache/county_farm_emp.rds"))
