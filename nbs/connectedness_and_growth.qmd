...
.prep_data_county_farm_emp <- function() {
...
# file.remove(file.path(root_dir, "cache/prep_data_county_farm_emp.rds"))
prep_data_county_farm_emp <- reseng$cache(.prep_data_county_farm_emp, file.path(root_dir, "cache/prep_data_county_farm_emp.rds"))
...
data_prep_county_emp <- reseng$cache(.data_prep_county_emp, file.path(root_dir, "cache/data_prep_county_emp.rds"))

head(data_prep_county_emp())
```
...
data_prep_rurality <- function() {
  
  # full list of county FIPS
  df <- data_prep_county_id() %>%
    select(stcty)
...
head(data_prep_rurality())
```


## sector output

Note that this output is in nominal dollars.

```{r}
..data_prep_county_output <- function(year, ilevel) {
  d <- conn$total_output(as.character(year), ilevel)
  d <- d %>%
...
.data_prep_county_output <- reseng$cache(..data_prep_county_output, file.path(root_dir, "cache/county_output_{ilevel}_{year}.rds"))

data_prep_county_output <- reseng$cache(function() {
  years <- 2001:2020
  d <- lapply(years, .data_prep_county_output, ilevel = "sec")
  # parallel execution on 4 CPUs is not much faster
  # d <- parallel::mclapply(years, .data_prep_county_output, ilevel = "sec")
  d <- do.call(rbind, d)
  d
}, "cache/county_output.rds")

head(data_prep_county_output())
```

## connectedness

```{r}
data_prep_imped <- reseng$cache(function(kind, par) {
  tiger_year = 2017
  if (kind == "dprox") {
...
data_prep_conn <- reseng$cache(function(year, imped_key) {
  if (imped_key == "none") {
    imped_mat <- NULL
  } else {
    x <- str_split_1(imped_key, "-")
    imped_mat <- data_prep_imped(x[1], x[2])
  }
...
years <- seq(2002, 2017, 5)
df <- data_prep_county_emp() %>%
  filter(year %in% years)

d <- data_prep_county_id()
df <- inner_join(df, d, "stcty")

d <- data_prep_rurality()
df <- inner_join(df, d, "stcty")

## share of ag in total output
d <- data_prep_county_output() %>%
  filter(year %in% years) %>%
  group_by(year, stcty) %>%
  mutate(output = sum(output_nom), output_share_ind11 = 100 * output_nom / output) %>%
  filter(ind == "11") %>%
  select(year, stcty, output, output_share_ind11)
...
df <- df %>%
  arrange(stcty, year) %>%
  group_by(stcty) %>%
  mutate(across(c(est, emp), 
                ~ 100 * (1 + 2 * (lead(.x) - .x) / (lead(.x) + .x)) ^ (1 / (lead(year) - year)) - 100, 
                .names = "{.col}_gr")) %>%
  ungroup()
...
f <- function(d) {
  ...
  arrange(stcty, year) %>%
  group_by(stcty) %>%
  mutate(conn_gr = (lead(conn) - conn) / (lead(year) - year)) %>%
  ungroup()
}
  
## max output absorption with no impedance
dfa_conn[["output, max abs, no imped"]] <- years %>%
  map(~ data_prep_conn(year = .x, imped_key = "none")) %>%
  do.call(rbind, .) %>%
...
dfa_conn[["output, max abs, dprox-300"]] <- years %>%
  map(~ data_prep_conn(year = .x, imped_key = "dprox-300")) %>%
  do.call(rbind, .) %>%
  select(year, stcty, max_absorption_alpha) %>%
  rename(conn = max_absorption_alpha) %>%
  f()
...
d <- dfa_stub %>%
  ...
  rename(conn_maxabs_noimp = conn, conn_maxabs_noimp_gr = conn_gr) %>%
  inner_join(dfa_conn[["output, max abs, dprox-300"]], c("year", "stcty")) %>%
  rename(conn_maxabs_dprox300 = conn, conn_maxabs_dprox300_gr = conn_gr)
...
- Total output as control variable, change to real dollars.
