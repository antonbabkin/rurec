...
data_prep$county_output_by_year <- reseng$cache(function(year, ilevel) {
  d <- conn_fns$total_output(as.character(year), ilevel)
  d <- d %>%
...
data_prep$county_output <- reseng$cache(function(ilevel) {
  years <- 2001:2020
  d <- lapply(years, data_prep$county_output_by_year, ilevel = ilevel)
  d <- do.call(rbind, d)
  d
}, file.path(root_dir, "cache/county_output/{ilevel}.rds"))
# file.remove(file.path(root_dir, "cache/county_output/det.rds"))
...
#' County intermediate supply by BEA industry (sector, summary or detail)
data_prep$intermed_supply_by_ind <- reseng$cache(function(year, ilevel) {
  d <- conn_fns$commodity_factor_supply_tidy_matrix(as.character(year), ilevel)
  d %>%
    as_tibble(rownames = "ind") %>%
...
}, file.path(root_dir, "cache/intermed_supply/{ilevel}_{year}.rds"))


#' County intermediate demand by BEA industry (sector, summary or detail)
data_prep$intermed_demand_by_ind <- reseng$cache(function(year, ilevel) {
  d <- conn_fns$commodity_factor_demand_tidy_matrix(as.character(year), ilevel)
  d %>%
    as_tibble(rownames = "ind") %>%
...
}, file.path(root_dir, "cache/intermed_demand/{ilevel}_{year}.rds"))

#' County intermediate supply and demand, total and agriculture&mining, aggregated from BEA detail industry
data_prep$intermed_sup_dem <- reseng$cache(function() {
  one_year <- function(year) {
    d1 <- data_prep$intermed_demand_by_ind(year, "det") %>%
      select(stcty, ind, intermed_demand)
    d2 <- data_prep$intermed_supply_by_ind(year, "det") %>%
  ...
  do.call(rbind, d)
}, file.path(root_dir, "cache/intermed_sup_dem_2001-2020.rds"))
# file.remove(file.path(root_dir, "cache/intermed_sup_dem_2001-2020.rds"))
...
`total_intermed_supply / total_output`

## excess S&D

`total_excess_intermed_supply` and `total_excess_intermed_demand`
