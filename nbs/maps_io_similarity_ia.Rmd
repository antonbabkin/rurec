...
trimr(specname=Output_mat_H1, macc=macc, singcc=singcc, ext=NA, industry_levels=industry_levels)
...
Queeg_selectr(specname=Output_mat_H2, CoreMatch=CoreMatch, MatchMat=MatchMat, cuml_core_out=cuml_core_out, queeg_mat=Queeg_rel_H1, imp_mat=Impede_mat[[4]], macc=macc, singcc=singcc, industry_levels=industry_levels, out_mat=Output_mat_H1)
```
...
hierarchr(specname=h1m, mat_mat=mat_mat, spec_clust=spec_clust, qdf=qdf, ext=ext, isolation_th=isolation_th) 
...
If maximum "connectedness" is below 10% a county is isolated. 
...
trimr(specname=Output_mat_H1, macc=macc, singcc=singcc, ext=ext, industry_levels=industry_levels)
...
If "connectedness" is below 10% a county is isolated. 
...
####Economic and Spatial cluster matching with isolation 
  mat_mat <- MatchMat
  spec_clust  <- union(setdiff(union(micro, metro), unlist(ext)), shoname)
  qdf <- Queeg_rel_H1
  isolation_th <- .1
  ext <- list(Masoncity = Masoncity,
              Burlington = Burlington,
...
# macc <- h1m[[3]]$NAME[h1m[[3]]$lab=="Isolated"]
# 
# #Each new core cluster 
# df <- vector(mode='list', length=length(setdiff(unique(h1m[[3]]$lab) , "Isolated")))
#   names(df) <- setdiff(unique(h1m[[3]]$lab) , "Isolated")
# 
# for(i in 1:length(setdiff(unique(h1m[[3]]$lab) , "Isolated"))){
#   df[[i]] <- h1m[[3]]$place[h1m[[3]]$lab == setdiff(unique(h1m[[3]]$lab) , "Isolated")[i]]
# }
#   
# ext <- df
# 
# #singcc <- unlist(df)
# singcc <- setdiff(unlist(ext), h1m[[3]]$place[h1m[[3]]$NAME %in% setdiff(unique(h1m[[3]]$lab) , "Isolated")])
# 
# 
# trimr(specname=Output_mat_H3, macc=macc, singcc=singcc, ext=ext, industry_levels=industry_levels)
# 
...
# 
# ############ Input Needs
# Input_Needs(specname=Output_mat_H3, o_mat=Output_mat_H1, industry_levels=industry_levels)
# 
# ############ Import Input Needs
# Import_Needs(specname=Input_mat_imp_H1, o_mat=Output_mat_H1, i_mat=Output_mat_H3,  industry_levels=industry_levels)
# 
# ############ Net Exports
# Export_Needs(specname=Input_mat_exp_H1, o_mat=Output_mat_H1, i_mat=Output_mat_H3,  industry_levels=industry_levels)
# 
# ############ Relative Queeg specification
# Rel_Queeg(specname=Queeg_rel_H1, o_mat=Output_mat_H1, im_mat=Input_mat_imp_H1, ex_mat=Input_mat_exp_H1, industry_levels=industry_levels)
