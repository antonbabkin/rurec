...
Industry_Count <- 3
Toy_Direct_mat <-  matrix(c(0.15,  0.20, 0.25, 
                            0.05,  0.30, 0.50,
                            0.20,  0.07, 0.10), nrow = Industry_Count, byrow = TRUE)
...
Toy_Total_mat %>% round(3)

```


"Industry-by-Location Payroll Table": 
  
```{r}
Region_Count <- 3

Toy_X_mat <-  matrix(c(65, 30, 35, 
                       99, 20,  25,
                       40, 15,  75), nrow = Industry_Count, byrow = TRUE)
...
"Net Import Shortage Table":  $\max \{\mathbf{AX}  - \mathbf{X}, 0\}$ 
...
"Net Input Excess  Table":  $\max \{\mathbf{AX}  - \mathbf{X}, 0\}$ 
...
"Relative Input Needs Table":  $\mathbf{AX}\left(\mathbf{\widehat{iAX}}\right)^{-1}$
...
"Relative Net Import Shortage Table":  $\max \{\mathbf{\widetilde{AX}} - \mathbf{\widetilde{X}}, 0\}$
...
#diag(as.vector(Toy_Transactions_mat %*% rep(c(1), each=ncol(Toy_Transactions_mat))))
...
colnames(Toy_Queeg) <- c("AllElse", "Brown", "Cook", "Dane")
Toy_Queeg  %>% round(4)
