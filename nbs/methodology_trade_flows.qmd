...
tf$plot_heatmap(x)
tf$plot_heatmap(floor(x))
tf$plot_heatmap(floor(x), discrete = TRUE)
```
...
```{r}
x <- tf$prep_lp_solver_inputs("491000")
...
```{r}
solution_deviation_summary <- function() {
  source("R/trade_flows.R", local = (tf <- new.env()))
  sup_mat <- readRDS(glue(tf$opath$supply_, .envir = tf$params_econ))
  dem_mat <- readRDS(glue(tf$opath$demand_, .envir = tf$params_econ))
  exsup_mat <- pmax(sup_mat - dem_mat, 0)
  exdem_mat <- pmax(dem_mat - sup_mat, 0)
  
  d <- data.frame()
  for (ind_code in rownames(sup_mat)) {
    x <- tf$call_trade_flows(ind_code)
    x <- x[rowSums(x) > 0, colSums(x) > 0, drop = FALSE]
    if (length(x) == 0) {
      d <- rbind(d, list(ind = ind_code, sup = 0, tf = 0, rmsd = 0, maxad = 0, maxrd = 0))
      next
    }
    sup <- exsup_mat[ind_code, rownames(x)]
    dem <- exdem_mat[ind_code, colnames(x)]
    sol_sup <- rowSums(x)
    sol_dem <- colSums(x)
    dev <- c(sol_sup, sol_dem) - c(sup, dem)
    ...
    row <- list(
      ...
      maxad = max(abs(dev)),
      maxrd = finmax(abs(dev) / c(sup, dem))
    )
    d <- rbind(d, row)
  }
  d <- d |>
    mutate(abs_dev = abs(tf - sup),
           rel_dev = replace_na(abs_dev / sup, 0),
           .after = "tf")
}
```
